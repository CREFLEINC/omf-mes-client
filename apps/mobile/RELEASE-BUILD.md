...
배포용 APK 를 빌드할 때 참고한다. 디버그 APK 는 개발 기기에서만 동작하므로, 배포에는
서명된 릴리스 APK 를 사용한다.
...
⛔ **서명 키와 비밀번호를 이 저장소에 두지 않는다.** 공개 저장소다. 키가 유출되면 제3자가
동일한 서명으로 앱을 배포할 수 있으며, 한 번 push 되면 회수할 수 없다. `build.gradle` 은
값을 환경변수로만 참조한다.

### 별도 준비물 배치

전달받은 파일과 값을 아래 경로에 배치한다. 세 항목 모두 저장소 외부 경로이거나 커밋 대상이
아닌 경로다.
...
cp <전달받은-경로>/omf-mes-client-android-release.jks ~/.secrets/
chmod 600 ~/.secrets/omf-mes-client-android-release.jks
```

다른 경로에 배치하는 경우 빌드 시 `OMF_RELEASE_KEYSTORE` 로 지정한다.
...
`-w` 뒤에 값을 지정하지 않으면 입력·재입력 프롬프트가 표시된다. 이 방식으로 실행해야 값이
셸 히스토리와 `ps` 목록에 남지 않는다.

macOS 가 아닌 환경에는 Keychain 이 없다. 빌드 시 `OMF_RELEASE_KEYSTORE_PASSWORD` 로 직접
전달한다.
...
값의 형식은 아래 「API 주소 설정」을 참고한다. 이 파일은 `.gitignore` 의 `*.local` 규칙에
해당하여 커밋되지 않는다.
...
세 항목이 모두 준비되면 빌드할 수 있다. 누락된 항목이 있으면 스크립트가 해당 항목을 명시하고
중단한다.

## 키스토어 생성

최초 1회만 수행한다. 키스토어를 전달받았다면 이 절은 해당하지 않는다.

**비밀번호를 명령 인자로 지정하지 않는다.** `ps` 목록과 셸 히스토리에 남는다.
...
Keychain 에 먼저 등록한 뒤 키스토어를 생성한다. 순서가 반대이면 중간에 실패했을 때 열 수 없는
파일만 남는다.
...
지문이 아래 값과 일치해야 한다. 일치하지 않으면 다른 키이며, 해당 키로 빌드한 APK 는 기존
설치를 갱신할 수 없다.
...
에뮬레이터와 시스템 이미지는 빌드에 필요하지 않다. 실기에서만 확인한다면 설치하지 않아도 된다.

## API 주소 설정

IP 와 포트는 `apps/mobile/.env.local` 한 줄로 지정한다. 이 파일은 `.gitignore` 의 `*.local`
규칙에 해당하여 커밋되지 않는다. 사내 주소를 저장소에 기록하지 않는다.
...
빌드 스크립트와 웹 빌드가 모두 이 값을 참조한다. 명령에 직접 지정하면 그 값이 우선한다.

| 항목 | 위치 |
| --- | --- |
| IP · PORT | `apps/mobile/.env.local` 의 `VITE_API_BASE_URL` (예시는 `.env.example`) |
| 경로 접두 | 동일 값의 끝부분(`/api`). 서버에 접두가 없으면 생략한다 |
| 버전 | `android/app/build.gradle` 의 `versionCode` · `versionName` |
| 키스토어 경로 | 환경변수 `OMF_RELEASE_KEYSTORE` (기본 `~/.secrets/...jks`) |

⚠ **이 값은 빌드 시점에 APK 에 고정된다.** 설치 후에는 변경할 수 없으므로, 주소가 변경되면
다시 빌드한다. 값을 지정하지 않으면 스크립트가 중단된다. 기본값 `127.0.0.1` 은 단말 자신을
가리키므로 빌드와 설치는 성공하고 조회만 모두 실패한다.

앱 설치 전에 서버 상태를 먼저 확인한다. 인증 없이 호출할 수 있는 경로는 상태 확인 하나다.
...
산출물 이름이 `app-release-unsigned.apk` 이면 서명되지 않은 것이며 단말에 설치할 수 없다.
...
| 계층 | 설정 | 범위 제한 |
| --- | --- | --- |
| 시스템 정책 | `network_security_config` | **호스트 단위로 제한된다.** 나머지 주소는 차단된다 |
| WebView 정책 | `allowMixedContent` (`CAP_ALLOW_CLEARTEXT_HTTP`) | 불가. WebView 전체에 적용된다 |

앱이 `https://localhost` 에서 동작하므로 두 계층을 모두 허용해야 통신된다. 시스템 정책만
허용하면 혼합 콘텐츠로 차단되고, WebView 만 허용하면 시스템 정책에서 차단된다.
...
# networkSecurityConfig 만 출력되어야 한다
```

Capacitor 는 `usesCleartextTraffic="true"` 를 추가하는데, 이는 주소를 구분하지 않는 전면 허용이다.
...
스크립트로 빌드한 경우 이 검증이 마지막 단계에 포함되어 있다. 출력된 지문이 「키스토어 지문
확인」의 값과 일치해야 한다.

### 서명 방식

키는 하나다. `v1`~`v4` 는 **동일한 키를 APK 에 기록하는 형식**의 차이이며, 안드로이드 버전마다
읽을 수 있는 형식이 다르다. 네 가지를 `build.gradle` 에 모두 명시한다.

| | 사용 | 이유 |
| --- | :-: | --- |
| v1 (JAR) | ✗ | `minSdk 33` 미만을 위한 하위 호환이라 대상이 없다 |
| v2 | ✓ | APK 전체를 서명한다 |
| v3 | ✓ | **키 교체 계보**를 기록한다. 없으면 이후 키를 교체할 수 없다 |
| v4 | ✗ | `adb --incremental` 전용. 사내 반입 설치와 무관하다 |

⚠ **네 가지를 모두 명시해야 한다.** AGP 는 아무것도 지정하지 않으면 `minSdk` 를 기준으로
자동 결정하는데, 하나라도 명시하면 자동 결정을 중단하고 나머지를 비활성화한다.
`enableV3Signing` 만 지정하면 v2 블록이 누락된다.
