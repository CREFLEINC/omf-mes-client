 ...
 * 단말 토큰이 싣고 오는 것을 읽는다.
 *
 * ⭐ **토큰에 있는 것은 단말 «번호» 하나뿐이다**(`sub`). 단말 코드·공장은 이 번호로 서버에
 * 물어본다 — P-CO-01 이 같은 길을 쓴다(`patterns/pop-registration`).
 *
 * ⛔ **코드·공장이 토큰에 들어 있다고 보지 않는다.** 종전에는 `terminalCode` 와 `plantId` 를
 * 요구하고 둘이 없으면 등록 QR 이 아니라고 판정했는데, 실제 토큰에 그 둘이 없어 **모든 등록
 * QR 이 걸러졌다.** 화면은 읽은 값을 조용히 버린 채 스캔 대기에 머물렀고, 현장에서는
 * 「카메라는 열렸는데 인식이 안 된다」로 보였다(#1103).
 *
