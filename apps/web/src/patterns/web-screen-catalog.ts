 ...
 *
 * ⛔ **사이드바에 없는 화면은 담지 않는다.** 이 목록의 불변식이 「메뉴 항목 전부가 여기 있다」이고,
 * 메뉴 밖 화면(로그인 `W-CO-01` · 신규 P/O 등록 `W-01-11` · 완제품 재고 조회 `W-04-08`)을 섞으면
 * 그 불변식이 「무엇의 전부인가」를 잃는다. 그 화면들에 코드가 필요해지면 **왜 메뉴에 없는지**를
 * 적은 별도 목록으로 세운다(POP 이 진입 화면을 그렇게 갈랐다).
 */
