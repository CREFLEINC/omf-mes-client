 ...
 * 이동」 자리에 선다.
 *
 * ⭐ **왜 있는가.** 갈 곳인 작업 시작 화면(P-02-01)이 아직 이 저장소에 없어 그 버튼은 눌러도
 * 아무 일도 하지 않는다. 그동안 POP 화면을 손으로 확인하려면 주소를 직접 쳐야 하는데,
 * 단말 셸에는 주소창이 없어 개발 중에도 확인 경로가 사람마다 갈렸다. 이 셀렉터가 그 자리를
 * **개발 중에만** 메운다.
 *
 ...
 * 개발용 조작이 남는다.
 *
 * ⛔ **P-02-01이 서면 이 파일과 호출부를 함께 걷는다.** 그때 그 버튼은 실제 이동 대상을
 * 갖게 되므로 이 대체물이 남아 있을 이유가 없다.
 */
...
export const POP_DEV_SCREENS: SelectOption[] = [
  ...
  /* ⚠ 화면 파일이 화면 ID를 밝히지 않는다 — 지어내지 않고 이름만 적는다. */
  { value: '/pop/rework-results', label: '재작업 실적 등록' },
];
