...
describe('사이드바 항목 높이', () => {
  ...
  /**
   * ⛔ **묶음 손잡이에 높이를 못 박지 않는다.** #1077 로 찌그러진 것은 명시 `height` 를 가진 DS
   * 항목뿐이었고, 손잡이가 오늘 안전한 이유도 높이를 적지 않아 내용 최소 크기가 축소를 막기
   * 때문이다. 항목과 줄을 맞추려고 높이를 적는 날 **같은 결함이 손잡이로 돌아온다** — 줄맞춤은
   * 아이콘 자리 폭(`.nav-group-chevron`)으로 잡는다.
   */
  it('묶음 손잡이도 높이를 못 박지 않는다', () => {
    expect(rules).not.toMatch(/\.nav-group-toggle\s*\{[^}]*height:/u);
  });
});
