...
describe('formatDateTime', () => {
  it('날짜와 시각을 공장 시각(UTC+7)으로 낸다', () => {
    expect(formatDateTime('2026-08-06T09:12:00+09:00')).toBe('2026-08-06 07:12');
  });

  /*
   * 공장 시각(UTC+7)으로 옮긴다(omf-all-around#20). 보는 사람의 실행 환경 시간대와 상관없이
   * 같은 순간이면 offset이 달라도 같은 시각이다 — 자정을 넘으면 날짜도 바뀐다.
   */
  it('같은 순간이면 offset이 달라도 같은 공장 시각을 낸다', () => {
    expect(formatDateTime('2026-08-06T23:50:00+00:00')).toBe('2026-08-07 06:50');
    expect(formatDateTime('2026-08-07T01:50:00+02:00')).toBe('2026-08-07 06:50');
