...
/**
 * **「이 줄을 반품할 수 있는가」를 판정하는 유일한 자리**(계획 §4.1 · 읽는 자리 파생).
 *
 * 표·화면·요청 조립이 각자 판정하면 규칙이 갈린다 — 표에는 고를 수 있다고 나오는데 보낼 때
 * 막히거나, 반대로 보낼 수 없는 줄이 요청에 실린다. **재고를 차감하는 되돌릴 수 없는 쓰기**라
 * 그 어긋남은 잘못된 반품 전표로 남는다.
 *
 ...
 *
 * **보류(`held`)로도 판정하지 않는다.** 보류된 자재 LOT을 되돌려 보내는 것이 이 화면의 주
 * 용도다 — 표식은 알리는 것이고 막는 것이 아니다(착수 이슈 §6).
 *
 ...
 * 얼마든 반품 라인을 만들 수 없다. 사용자가 헛다리를 짚지 않게 한다.
 *
 * - **값 없음**: 계약이 반품 라인 한 줄에 품목·자재 LOT·수량·단위·출발 위치 **다섯을 전부
 *   필수**로 두었다(실측). 적치 목적지가 곧 반품의 출발 위치다.
 * - **수량 0 이하**: 계약이 `issueQty`에 `exclusiveMinimum: 0`을 두어 **0도 보낼 수 없다.**
 *   재고실사(`minimum: 0`)와 갈리는 자리다 — 그쪽은 「세어 보니 하나도 없었다」가 정상 값이지만
 *   여기서는 「0개를 되돌려 보낸다」에 뜻이 없다.
 */
