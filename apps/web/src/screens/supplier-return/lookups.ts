...
export const lookupKeys = {
  ...
  itemName: (itemId: number) => ['supplier-return-lookups', 'item-name', itemId] as const,
 ...
 * ⭐ 부르는 횟수는 **보이는 줄이 가리키는 품목 수**다 — 자재 LOT과 같은 짜임이라
...
export interface ItemNameLookup {
   ...
   * 둘을 뭉개면 지워진 품목 한 건이 구획 전체에 「다시 시도」를 세우고 영영 풀리지 않는다.
   */
  of: (itemId: number) => ReferenceState;
  /** 하나라도 **못 받았는가**(404는 아니다). 실패 안내와 「다시 시도」가 본다. */
  isError: boolean;
  refetch: () => void;
}
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      fetchAllPages((page, size) =>
        runRequest(() => client.GET('/mdm/partners', { params: { query: { page, size } } })),
      ),
  });
