 ...
 * 사유·비고는 열로 만들지 않았다 — 이 화면의 판단(무엇을 되돌려 보낼까)에 쓰이지 않고,
 * 원천 문서는 낼 것이 번호밖에 없다(#44).
 *
 * **정렬을 열지 않는다.** 계약의 입고 목록 조회에 정렬 파라미터가 **없고**(실측) 화면 안에서만
 * 정렬하면 「지금 쪽 안에서만 정렬됐다」는 사정을 매번 설명해야 한다. 이 화면에서 대상을
 * 찾는 정상 경로는 입고번호 검색과 창고·기간 조건이다.
 *
 ...
 * | 열 | 폭 | 근거 |
 * | --- | ---: | --- |
 * | 입고번호 | 168px | `GR-2026-000310` 14자(자폭 약 7.5px) 105px + 셀 여백 32px + 여유 |
...
export const buildGrColumns = ({
  ...
  },
  {
     ...
     * 것이 된다. 코드는 번역하지 않고 그대로 낸다 — 칩으로 감싸지 않는 이유는 비활성 표현이
     * 필요한 자리가 생겼을 때 칩이 그 prop을 갖고 있지 않기 때문이다(설치본 실측).
     */
  },
  {
    key: 'receiptDatetime',
 ...
 *
 * **빈 상태를 바깥에서 가르지 않는다**(M18). 표를 늘 그리고 `empty`가 0건을 맡는다 —
