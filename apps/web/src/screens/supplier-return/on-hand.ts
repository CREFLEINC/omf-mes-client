...
/**
 * **반품 수량의 상한을 정하는 유일한 자리**(계획 결정 9 · 완료 조건 C34).
 *
 ...
 * ---
 *
 * **상한이 위치까지 좁혀지지 않는다는 한계를 밝힌다**(어긋남 6 · 승인 13-6).
 * `/inventory/balances`는 `groupBy` 축 **하나만** 채워 내리므로 LOT으로 묶으면 위치 축이
 * 접힌다. 그래서 이 상한은 **그 창고 그 LOT의 합계**이고, 같은 LOT이 두 위치에 나뉘어 있으면
 * 화면 상한이 실제보다 **느슨하다.** 느슨한 쪽이 안전하다 — 화면이 정당한 반품을 막지 않고,
 * 초과분은 서버가 400으로 되돌린다(되돌릴 수 있는 실패다).
 *
 * **가용 수량을 쓰지 않는다**(감지기 M32). `availableQty`는 보유에서 예약·피킹·**보류**를 뺀
 * 값인데, 보류된 자재 LOT을 되돌려 보내는 것이 이 화면의 주 용도다 — 상한으로 쓰면
 * **반품해야 할 것을 막는다.** 화면 타입에 그 자리조차 두지 않았다(`types.ts`).
 *
 ...
 * 갈라 말하기 위해서이고, 무엇보다 **실패를 「목록에 없음」으로 뭉개지 않기** 위해서다(#47).
 */
 ...
 * 그 줄의 보유 수량.
 *
 * **세 갈래다**(승인 13-6) — 확인함 · 아직 오지 않음 · 확인하지 못함. 못 구한 것을 `0`이나
 * `Infinity`로 읽으면 정당한 반품을 막거나 상한이 아예 없어진다(감지기 M23).
 */
export type OnHand =
...
export const toOnHand = (source: BalanceSource, line: ReceiptLineView): OnHand => {
  ...
  const rows = item.entries.filter((entry) => entry.lotId === line.lotId);
 ...
 * 친 수량이 상한 안인가.
 *
 * **확인하지 못한 줄은 막지 않는다**(승인 13-6). 상한은 보조 정보이고 최종 판정은 서버가
 * 한다 — 잔액 조회가 실패했다고 반품 업무 전체를 세우면, LOT이 많은 창고에서는 정당한 반품이
 * 영영 불가능해진다. 초과분은 400으로 되돌아오며 그 실패는 되돌릴 수 있다.
 */
