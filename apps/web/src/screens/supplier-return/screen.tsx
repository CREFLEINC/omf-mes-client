...
import { isReceiptNotFound, useGoodsReceiptDetail, useGoodsReceipts } from './queries';
 ...
 * 결과를 낸다.
 *
 * **이 회차에는 쓰기가 없다.** 반품 줄 선택·수량·반품 처리는 뒤따르는 회차에서 이 컨테이너에
 * 붙는다. 그때까지 이 화면은 **라우트에도 사이드바에도 등록되지 않는다** — 반품할 수 없는
 * 「공급사 반품 처리」를 노출하면 미완성 기능을 사용자에게 내보이는 것이다.
 *
 ...
 * | **S1** 전표를 골랐다 | `gr`이 있고 **상세가 200** | 위 + 제목줄 · 라인 표 | 위 + (다음 회차) 줄 고르기 | `+&gr` |
...
export const SupplierReturnScreen = () => {
   ...
   *
   * **뒤 네 열은 이 회차에 아직 없다.** 열을 지우지 않고 남겨 두는 이유는, 표에 오르지 않은
   ...
   *   되짚을 수 없다.
   * - **9행이 이 화면의 #43 자리다**: 응답 도착이 조건 입력을 되돌리면 「치던 값이 사라진다」가
