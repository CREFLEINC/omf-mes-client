...
export interface SelectFieldProps {
  ...
  wide?: boolean;
  /**
   * 이 칸이 잠겼는가. **반품 정보 구획에만 붙는다** — 조회 조건 칸은 잠그지 않는다(배치 규범 3).
   * 잠그면 지금 걸린 조건을 읽을 수만 있고 고칠 수 없는 상태가 된다.
   *
   * 반품 정보는 다르다: **전송 중에 값이 바뀌면 사용자가 확인한 것과 나가는 것이 갈린다.**
   */
  disabled?: boolean;
  /**
 ...
 * 잠긴 칸은 포커스를 받지 못해 툴팁만으로는 키보드·스크린리더 사용자가 닿을 수 없다.
 *
 * **잠금과 오류를 받는 자리는 반품 정보 구획이다.** 조회 조건 칸은 둘 다 쓰지 않는다 —
 * 조건은 잠그지 않고(배치 규범 3), 조건 값에는 서버가 오류를 돌려줄 자리가 없다.
 *
