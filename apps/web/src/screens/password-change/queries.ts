...
export const useChangePassword = (options: PasswordChangeOptions): PasswordChangeMutation => {
   ...
   * | 갈래 | 이 참조 | 왜 |
   * | --- | --- | --- |
   * | 통신 실패 · 가를 근거 없음 | **유지** | 적용됐는지 모른다 — 같은 키라야 서버가 두 번 실행하지 않는다. 특히 5xx는 **적용됐는데 응답만 실패한** 경우를 포함한다 |
   * | 401 | **유지** | 같은 값이면 같은 401이 옳고, 값을 고치면 새 키가 나간다(무해) |
   * | **400**(검증 실패) | **유지** | 계약이 이 갈래를 「검증 실패. 고쳐야 풀린다」로 두었다 — **실행 전 거부**라 적용되지 않았음이 확정이고, 401과 같은 이유로 유지가 무해하다. 이 갈래를 푸는 길은 값 수정뿐이고 그때 생성 규칙이 새 키를 만든다 |
