 ...
 * 그린 라벨을 **셸에게 넘기는 자리.**
 *
 * 화면은 프린터로 보내지 않는다 — 셸이 보낸다. 이 모듈이 하는 일은 화면과 셸 사이를 잇는 것뿐이다.
 *
 * ⚠ **자재 LOT 라벨은 화면이 짠다**(사용자 지시 2026-09-14 · `label-tspl` 머리말). 서버 렌디션이
 *   100 × 60 mm 좌표라 80 × 30 mm 라벨지에서 잘렸다. 이 모듈은 그 명령을 그대로 넘길 뿐 다시
 *   고치지 않는다.
 *
