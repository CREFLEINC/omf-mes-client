 ...
 * 조회가 실패했을 때 서는 배너.
 *
 * ⛔ **서버 문구를 잇기 «전»에 항목별로 거른다.** 저장소의 옛 사본들이 `join(' ')` 한 뒤 빈
 * 문자열인지 검사하는데, 그 형태는 공백만 있는 문구와 빈 문구 여럿에서 **공백만 그린 배너**를
 * 만든다(client#192 — 전수 확인된 결함). 올바른 형태의 전례는 `login/login-error-banner.tsx` 다.
 *
 * ⭐ **「다시 시도」는 다시 시도해서 달라지는 갈래에만 둔다**(공유계약 G-23). 권한 부족은 같은
 * 답이 오므로 두지 않는다 — 누를 수 있는데 아무 일도 없는 컨트롤은 사용자를 헛돌게 하고 정작
 * 해야 할 일(담당자 문의)을 가린다. 뒤집어서, **「다시 시도하세요」라고 말하는 갈래에는 반드시
 * 누를 자리가 있어야 한다.**
 *
...
export interface QueueLoadErrorBannerProps {
  error: ApiError;
  onRetry: () => void;
}

export const QueueLoadErrorBanner = ({ error, onRetry }: QueueLoadErrorBannerProps) => {
