...
describe('toApprovalTypeOptions', () => {
  ...
  it('값이 차면 코드값을 그대로 라벨로 쓴다', () => {
    ...
    expect(toApprovalTypeOptions(['PURCHASE_ORDER', 'INVENTORY_ADJUSTMENT'])).toEqual([
  ...
  it('차례를 바꾸지 않는다', () => {
    ...
    expect(
      toApprovalTypeOptions(['INVENTORY_ADJUSTMENT', 'PURCHASE_ORDER']).map((o) => o.value),
    ).toEqual(['INVENTORY_ADJUSTMENT', 'PURCHASE_ORDER']);
  });
});
...
describe('codeNote · codePlaceholder', () => {
  ...
  it('선택지가 차면 안내를 거둔다', () => {
    // 남으면 화면이 거짓말을 한다.
    expect(codeNote(toApprovalTypeOptions(['PURCHASE_ORDER']))).toBeUndefined();
  });
});
