 ...
 *
 * 상태 코드는 `MASTER_VERSION_STATUS`의 `DRAFT`·`CONFIRMED`·`OBSOLETE` 3값으로
 * 확정됐다(설계 2026-09-03). 이전 응답에서 쓰던 한글 값은 전환 기간 호환을 위해 함께 읽되,
 * 화면의 판정 지점은 아래 세 상수로 한정한다.
 *
 ...
 *   데이터가 손상되지 않는다.
 *
 * 즉 **서버 400이 최종 방어선**이고 이 매핑은 예기치 않은 구버전 응답을 견디기 위한 방어다.
 *
