 ...
 * | 자리 | 값 | 근거 |
 * | --- | --- | --- |
 * | **`businessDate`** | **사용자가 확인한 영업일**(기본값은 제출 순간의 날짜) | 공유계약 C-8 — 「클라이언트가 `business_date`까지 정한다」. **산출 규칙(야간조 경계)이 확정되지 않았다**(미결 대장) |
 * | **`occurredAt`** | **사용자가 확인한 발생 일시**(기본값은 제출 순간) | 공유계약 C-1 규칙 3 — 발생과 수신을 가른다. 어긋난 것을 발견한 시각과 전기를 누른 시각은 다를 수 있다 |
