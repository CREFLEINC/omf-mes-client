 ...
 *
 * **사용 여부는 판정 근거가 아니다 — 꺼진 그룹도 찾는다.** 끄면 그 그룹의 코드값을 아예
 * 받지 못해 사유가 통째로 사라지고, 사용자에게는 「값이 하나도 없다」와 구분되지 않는다.
 *
 * ⚠ **찾되 알리지는 않는다.** 전례 W-06-04는 꺼진 그룹에 경고 배너를 세우지만, 이 화면에는
 * 그 자리가 없다 — 이 화면 사용자는 마스터를 고칠 사람이 아니고, 그 안내는 지시 ④가 금지한
 * 「화면이 미완성이다」로 읽힐 소지가 있다(#36 회신 ④). 세울지 여부는 설계 판단이다.
 */
...
/**
 * 그룹 조회 — **첫 쪽만 본다.** 그룹코드로 좁힌 조회라 다음 쪽까지 갈 결과가 나오지 않는다.
 *
 * ⚠ **그룹 목록의 잘림은 밝히지 않는다**(코드값만 `truncated`를 낸다). 계약의 `q`가 그룹코드와
 * 그룹명을 함께 검색해(계약 실측) 정확 일치 그룹이 첫 쪽 밖으로 밀리려면 같은 문자열을 품은
 * 그룹이 쪽 크기만큼 있어야 한다 — 전례도 같은 판단이다. 그 일이 실제로 생기면 화면은
 * 「없음」으로 조용히 떨어진다.
 */
 ...
 *
 * **코드값 조회에는 `includeInactive`를 켜지 않는다**(⚠ 위 그룹 조회는 켠다 — 그쪽은 판정이
 * 목적이라 꺼진 그룹도 찾아야 한다). 다섯 참조(창고·위치·품목·단위·LOT)는 지난 자료의 이름을
