...
describe('readReason', () => {
   ...
   * 통과시킨다**(계획 §5.2.4 · 이번 회차 목 4021 재실측) — 통과하면 결재함 목록의 요약 자리가
  ...
  it('앞뒤 공백을 떼고 보낼 값을 만든다', () => {
    expect(readReason('  실사 차이분 조정  ')).toEqual({
      kind: 'ready',
      reason: '실사 차이분 조정',
      firstLine: '실사 차이분 조정',
    });
  ...
  it('짧은 사유도 보낼 수 있다', () => {
    expect(readReason('실사')).toEqual({ kind: 'ready', reason: '실사', firstLine: '실사' });
  });
});
...
describe('toApprovalRequest', () => {
   ...
   * 계약 밖의 요청이 된다 — 승인 주체는 결재선 정의(W-06-15)가 정하지 이 화면이 정하지 않는다.
   *
   * ⭐ **헤더 사유(`reasonCode`)를 여기 싣지 않는다**(D-8). 그것은 **코드**이고 이것은 **자유
   * 텍스트**다 — 섞으면 결재함 목록의 요약 자리에 코드가 선다.
   */
  ...
  /** 헤더 사유 코드를 섞어 보내는 길을 막는다 — 본문 키가 늘지 않는다. */
  it('본문에 조정 사유 코드가 실릴 자리가 없다', () => {
    expect(Object.keys(toApprovalRequest('실사 차이분 조정') ?? {})).not.toContain('reasonCode');
  });
});
...
describe('toReasonLines · toVisibleLine', () => {
  /**
   * **상수 자신을 기대값으로 쓰는 고리를 끊는다**(사본 체크리스트 6번).
   *
