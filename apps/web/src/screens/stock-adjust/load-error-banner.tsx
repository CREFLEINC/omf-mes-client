 ...
 *
 * **조회 실패 전용이다.** 등록·상신·전기(쓰기)의 실패는 갈래가 다르고(검증 400 · 권한 403 ·
 * 충돌 409 · 응답 없음) 공통 `SaveErrorBanner`가 맡는다 — 그 배선은 뒤따르는 회차에 있다.
 * 둘을 한 부품으로 합치지 않는 이유는 사용자가 할 조치가 다르기 때문이다.
 *
 ...
 *
 * **실패를 빈 상태로 보이지 않는다.** 「없습니다」로 내면 사용자가 자료가 없는 줄 알고 다른 곳을
 * 뒤진다 — 실제로는 조회 자체가 되지 않은 것이라 무엇을 해도 결과가 같다.
 *
