...
describe('formatDateTime', () => {
  it('날짜와 분까지만 공장 시각(UTC+7)으로 낸다', () => {
    expect(formatDateTime('2026-08-18T14:35:00+09:00')).toBe('2026-08-18 12:35');
  });

  /** 공장 시각(UTC+7)으로 옮긴다(omf-all-around#20) — 같은 순간이면 offset이 달라도 같은 시각이다. */
  it('같은 순간이면 다른 offset이 와도 같은 공장 시각을 낸다', () => {
    expect(formatDateTime('2026-08-18T07:35:00Z')).toBe('2026-08-18 14:35');
    expect(formatDateTime('2026-08-18T16:35:00+09:00')).toBe('2026-08-18 14:35');
  });

  it('다른 순간이면 다른 시각을 낸다', () => {
    expect(formatDateTime('2026-08-18T14:35:00Z')).toBe('2026-08-18 21:35');
  });
