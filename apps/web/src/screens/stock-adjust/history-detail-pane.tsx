 ...
 *
 * **코드를 뜻으로 옮기지 않는다**(공유계약 G-2) — 서버가 준 글자를 그대로 낸다. 화면이 뜻을
 * 붙이면 그 뜻은 화면이 지어낸 것이 되고, 값이 바뀌는 날 조용히 틀린다.
 *
 * ⚠ **사유는 이제 이름을 가질 수 있다** — 마스터 조회가 `codeName`을 함께 준다(#36 회신).
 * 그 이름을 이 표에도 붙일지는 **설계 판단**이라 이 파일에서 정하지 않는다(상태는 그 이름을
 * 줄 자리 자체가 아직 없다).
 *
