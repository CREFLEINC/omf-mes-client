 ...
 *
 * ⚠ **짝인 `IN_SERVICE` 를 두지 않았다** — 이 화면에는 그것으로 판정할 자리가 없다.
 * 목록 조회에 상태 조건이 계약에 없어(서버가 「기본은 유효한 것만」 내린다) 형제 화면처럼
 * 「운용 중만 보기」를 걸 수 없기 때문이다. 쓰지 않는 이름은 두지 않는다.
 */
export const DISPOSED_STATUS_CODE = 'DISPOSED';

/**
 * 금형을 뜻하는 도구 유형 코드. **선택지가 아니라 판정에 쓰는 값**이라 이름을 갖는다.
 *
 * ⭐ 서버 공통코드 `TOOL_TYPE` 이 내주는 값이다(금형 `MOLD` · 지그 `JIG` · 그 밖 `OTHER` —
 *   2026-09-21 실측·시드). 화면이 지어낸 값이 아니다.
 * ⚠ 여기서는 **도움말을 언제 보일지**에만 쓴다 — 캐비티 칸을 잠그거나 값을 바꾸는 데 쓰지 않는다.
 */
export const MOLD_TOOL_TYPE_CODE = 'MOLD';
