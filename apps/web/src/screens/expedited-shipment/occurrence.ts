...
/**
 * 긴급 직행 출하 본문에 **언제 일어난 일인가**를 얹는다.
 *
 * 계약이 `ShipmentCreate`에 `businessDate`·`occurredAt`을 필수로 요구한다(공유계약 C-8).
 * 두 값의 뜻이 다르다 — `occurredAt`은 **이 화면에서 [출하 처리]를 확정한 순간**이고,
 * `businessDate`는 그 순간이 속한 **영업일**이다.
 *
 * ⚠ **영업일 산출 규칙(야간조 경계 등)은 아직 계약에 없다.** 이 화면의 출하는 누르는 그 자리에서
...
export type ShipmentCreateDraft = Omit<ShipmentCreate, 'businessDate' | 'occurredAt'>;
...
export const withOccurrence = (draft: ShipmentCreateDraft, now: Date): ShipmentCreate => ({
  ...draft,
