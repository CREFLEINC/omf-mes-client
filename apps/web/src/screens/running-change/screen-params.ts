 ...
 * 이 화면이 주소에서 읽는 값.
 *
 * **작업지시는 주소가 소유한다.** POP 에서 작업지시를 고르는 자리는 `P-02-01`(작업 시작)이고,
 * 러닝체인지는 그 뒤 메인 화면에서 인라인으로 들어온다(스펙 §2 프로세스). 그 화면들이 이
 * 저장소에 설 때까지 진입 경로는 주소뿐이며, 붙었을 때도 넘기는 방식은 같은 주소 키다.
 *
 * ⛔ **작업지시를 기억해 두지 않는다.** 앞서 본 작업지시를 화면이 들고 있으면 주소만 보고는
 * 무엇을 교체하는 화면인지 알 수 없게 되고, 단말을 넘겨받은 다음 작업자가 **남의 작업지시에
 * 교체를 기록한다** — 교체는 지워지지 않는다(§5-2 · 이력 불변 B-3).
 *
