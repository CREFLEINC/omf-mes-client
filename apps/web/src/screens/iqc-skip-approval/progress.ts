 ...
 *
 * 이 화면에서는 그 규율이 한 겹 더 무겁다 — 결재는 되돌릴 수 없고(계약에 취소 오퍼레이션이
 * 없다) 이 화면의 결재는 **검사를 건너뛰는 자물쇠를 푸는 일**이다.
 *
 * ---
 *
 * **결재 결과 코드를 해석하지 않는다.** 값 목록이 공통코드 소관이라(`omf-mes#64`) 어떤 코드가
 * 「승인」이고 「반려」인지 화면이 알 근거가 없다. 그래서 단계 상태는 **결재했는가·지금
 * 차례인가** 둘로만 갈리고, 반려는 **자리표시 집합**이 채워질 때 살아난다.
 *
 * 이 화면이 소유한다 — 다른 화면 슬라이스의 같은 이름 파일을 참조하지 않는다. 같은 계약을
 * 소비하는 화면이 이미 있으나(W-CO-09), 형태가 같아도 한 화면의 판정 변화가 다른 화면을
 * 끌고 가서는 안 된다.
 */
...
/**
 * 반려를 뜻하는 결재 코드 — **비어 있는 것이 지금의 사실이다.**
 *
 ...
 * 사용자에게는 사실로 보인다.
 *
 * **값이 확정되면 이 배열만 채우면 된다.** 판정은 이 배열을 읽는 자리 하나뿐이라 다른 곳을
 * 고칠 필요가 없고, 채우는 순간 그 코드의 단계가 반려로 그려진다.
 *
 * 추적: 반려 판별 필드 — **`omf-mes#64`**. 비공개 저장소이므로 번호로만 참조한다.
 */
 ...
 * 반려 자리표시에 들어 있을 때만 반려다 — 자리표시가 빈 지금은 어떤 코드도 반려가 되지 않는다.
 *
...
/** 결재 시각 표기. 실려 오지 않았으면 **없음을 값으로 낸다** — 빈 글자로 메우지 않는다. */
const decisionAtTextOf = (step: ApprovalStep): string | null => {
...
export const toStepProgressViews = (
  ...
  steps.map((step) => ({
    ...
    decisionAtText: decisionAtTextOf(step),
