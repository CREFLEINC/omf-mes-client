...
describe('toRejectBody', () => {
   ...
   * 여기서 통과시키면 반려 사유가 공백인 결재 기록이 남고, 상신자는 무엇을 고쳐야 하는지
   * 모른 채 같은 요청을 그대로 다시 올린다 — 이 화면의 반려 유도 문구가 겨눈 바로 그 자리다.
   */
  ...
  /** 여러 줄로 적은 반려 사유의 줄바꿈은 기록에 그대로 남아야 한다. */
  it('여러 줄 사유의 줄바꿈과 들여쓰기를 지운다거나 이어 붙이지 않는다', () => {
    expect(toRejectBody('첫 줄\n  둘째 줄')).toEqual({ comment: '첫 줄\n  둘째 줄' });
  });
});
...
describe('decisionBlockedReason', () => {
  ...
  /**
   * **잠금 판정이 대상 처리 현황이나 유형 코드를 보지 않는다.** 이 화면은 판단 근거를
   * 더 그리는 자리라 「근거가 덜 왔으니 막자」는 생각이 들기 쉬운데, 결재 가능 여부의
   * 정본은 서버가 준 `isMyTurn` 하나다 — 화면이 그 판정을 덮어쓰면 서버가 여는 것을 화면이 닫는다.
   */
  it('판정에 드는 값이 차례와 의견 둘뿐이다', () => {
    expect(decisionBlockedReason('approve', { isMyTurn: true, comment: '' })).toBeNull();
    expect(decisionBlockedReason('approve', { isMyTurn: false, comment: '' })).not.toBeNull();
  });
});
