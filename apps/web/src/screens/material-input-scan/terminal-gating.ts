 ...
 *
 * 스펙 §5-1이 ★로 둔 자리다. 계약의 `TerminalProcess.canInputMaterial`을 진입 시 읽어
 * 「투입 확정」을 선제 비활성한다. **기본이 닫힘**이므로 명시적 허용이 필요하다.
 *
 * ⛔ **이것은 보안 경계가 아니다.** 공유계약 F-1이 이 플래그를 「인증이 아니라 기능 구성」으로
 * 못박았다 — 없는 장비의 버튼을 감춰 오조작을 줄이는 장치다. **집행은 서버가 한다**(쓰기의
 * 403). 그래서 화면이 열어 준다고 통과하는 것이 아니고, 화면이 막는다고 그것이 방어인 것도
 * 아니다.
 *
 ...
 *
 * **행이 없으면 `false`와 같이 다룬다** — 구성되지 않은 공정은 열려 있지 않다. 계약이
 * 「기본은 닫힘」이라 적은 것과 같은 방향이다.
 */
...
export const useTerminalGate = (
  ...
  const verdict = ((): GateVerdict => {
    ...
    /*
     * 조회 중에는 **막는다.** 여는 쪽으로 두면 답이 오기 전 찰나에 눌린 확정이 게이팅을
