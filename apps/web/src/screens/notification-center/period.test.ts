...
describe('defaultPeriod', () => {
  ...
  it('돌려준 값은 그대로 조회할 수 있다 — 심자마자 막히면 첫 진입이 빈 화면이 된다', () => {
    expect(resolvePeriod(defaultPeriod(new Date(2026, 7, 6)), KST).kind).toBe('ready');
  });
...
describe('resolvePeriod — 조회하지 않는 갈래', () => {
  it('주소에 기간이 아예 없으면 비어 있는 것으로 본다 — 사용자가 잘못한 것이 없다', () => {
    expect(resolvePeriod({ from: '', to: '' }, KST)).toEqual({ kind: 'empty' });
  });

  it('없는 날짜는 막는다 — 자릿수만 보면 통과해 그대로 요청에 실린다', () => {
    expect(resolvePeriod({ from: '2026-02-31', to: '2026-08-06' }, KST)).toEqual({
      kind: 'blocked',
      reason: t.reasons.periodInvalid,
    });
    expect(resolvePeriod({ from: '2026-08-01', to: '2026-13-01' }, KST)).toEqual({
      kind: 'blocked',
      reason: t.reasons.periodInvalid,
    });
  });
  ...
  it('형식이 깨진 값도 같은 갈래다', () => {
    expect(resolvePeriod({ from: '2026-8-1', to: '2026-08-06' }, KST).kind).toBe('blocked');
    expect(resolvePeriod({ from: '20260801', to: '2026-08-06' }, KST).kind).toBe('blocked');
    expect(resolvePeriod({ from: '2026-08-01', to: '오늘' }, KST).kind).toBe('blocked');
  });

  it('한쪽만 채운 기간은 막는다 — 빈 쪽을 화면이 지어 채우지 않는다', () => {
    expect(resolvePeriod({ from: '2026-08-01', to: '' }, KST)).toEqual({
      kind: 'blocked',
      reason: t.reasons.periodInvalid,
    });
    expect(resolvePeriod({ from: '', to: '2026-08-06' }, KST)).toEqual({
      kind: 'blocked',
      reason: t.reasons.periodInvalid,
    });
  });
  ...
  it('종료가 시작보다 앞서면 다른 사유를 낸다 — 순서 문제를 날짜 문제로 말하지 않는다', () => {
    expect(resolvePeriod({ from: '2026-08-06', to: '2026-08-01' }, KST)).toEqual({
      kind: 'blocked',
  ...
  it('역전은 해가 바뀌어도 판정된다 — 문자열 앞자리로만 보면 놓친다', () => {
    expect(resolvePeriod({ from: '2026-01-02', to: '2025-12-31' }, KST).kind).toBe('blocked');
  });

  it('없는 날짜가 뒤집힘보다 앞선다 — 순서를 바꿔도 여전히 조회되지 않는 값이다', () => {
    expect(resolvePeriod({ from: '2026-02-31', to: '2026-01-01' }, KST)).toEqual({
      kind: 'blocked',
      reason: t.reasons.periodInvalid,
    });
  });
...
describe('resolvePeriod — 보내는 값', () => {
  it('시작은 그날 00:00:00, 종료는 그날 23:59:59다', () => {
     ...
     */
    expect(resolvePeriod({ from: '2026-08-01', to: '2026-08-06' }, KST)).toEqual({
      kind: 'ready',
      query: {
  ...
  it('같은 날 하루만 고른 것도 조회할 수 있다', () => {
    expect(resolvePeriod({ from: '2026-08-06', to: '2026-08-06' }, KST)).toEqual({
      kind: 'ready',
      query: {
  ...
  it('오프셋 0은 +00:00으로 적는다 — 시간대가 없으면 같은 글자가 지역마다 다른 순간이 된다', () => {
    const state = resolvePeriod({ from: '2026-08-01', to: '2026-08-01' }, at(0));
  ...
  it('UTC 서쪽은 부호를 그대로 낸다', () => {
    const state = resolvePeriod({ from: '2026-08-01', to: '2026-08-02' }, at(-300));
  ...
  it('30분 단위 오프셋도 분까지 적는다', () => {
    const state = resolvePeriod({ from: '2026-08-01', to: '2026-08-02' }, at(330));
  ...
  it('지금을 인자로 받는다 — 함수 안에서 실행 환경의 시각을 읽지 않는다', () => {
    ...
    expect(resolvePeriod({ from: '2026-08-01', to: '2026-08-06' }, morning)).toEqual(
      resolvePeriod({ from: '2026-08-01', to: '2026-08-06' }, evening),
    );
  });
});
