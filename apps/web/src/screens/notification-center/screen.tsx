 ...
 * 결과가 없을 때 쓰는 고정 참조.
 *
 * 렌더마다 새 배열을 만들면 내용이 같아도 참조가 달라, 이 값을 prop으로 받는 쪽이 매번
 * 달라진 것으로 본다. 이 회차에 실제 소비처가 생겼다 — `toPageView(…, rows.length)`와
 * 카드 목록이 함께 이 값을 읽는다.
 */
 ...
 *
 * **쪽을 되돌리는 자리는 4행 하나뿐이다**(`applyQuery`의 기본 인자). 여섯째가 생기면
 * 이 표에 행을 먼저 더한다.
 *
