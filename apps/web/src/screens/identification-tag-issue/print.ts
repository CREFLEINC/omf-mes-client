 ...
 */
export const TAG_RENDITION_FORMAT: LabelRenditionFormat = labelRenditionFormat();
...
export interface PrintTarget {
  ...
  /** 인쇄 작업 이름. 현장에서 어느 출력물인지 가리는 값이라 일련번호를 쓴다. */
  label: string;
}
 ...
 *
 * ⭐ **첫 실패에서 멈춘다.** 프린터가 죽은 상태로 480 장을 계속 밀면 480 번의 실패 보고가
 * 나가고 사용자는 그 사이 아무것도 할 수 없다. 멈춘 뒤 남은 것은 회차가 오르지 않은 채
 * 그대로 있고, 재인쇄가 이어받는다.
 *
