 ...
 * 준다(G-3).
 *
 * ⚠ **같은 플래그를 세 화면이 쓴다**(`P-02-05`·`P-02-07`·`P-02-09` — 스펙 §5-1). 그래도 이 파일을
 * 공용으로 올리지 않는다 — 화면 슬라이스는 사본으로 소유하는 것이 이 저장소의 관례이고, 아직
 * 두 번째 사용처가 서지 않았다.
 */
...
export const tagIssueGatingKeys = {
  /*
  ...
  processes: (terminalId: number, processId: number) =>
    ['identification-tag-issue', 'terminal-processes', terminalId, processId] as const,
};
...
export const useTerminalGate = (
  ...
  const query = useQuery({
    queryKey: tagIssueGatingKeys.processes(terminalId ?? 0, processId ?? 0),
    enabled: identified,
    queryFn: () => {
