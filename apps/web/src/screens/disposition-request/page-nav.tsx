...
export const PageNav = ({ view, label, onChange }: PageNavProps) => (
  <nav className="form-actions disposition-request-page-nav" aria-label={label}>
    ...
    >
      {messages.dispositionRequest.actions.prevPage}
    </Button>
    <Button
      variant="outlined"
      size="sm"
      ...
      onClick={() => onChange(view.page + 1)}
    >
      {messages.dispositionRequest.actions.nextPage}
    </Button>
  </nav>
);
