 ...
 *
 * 스펙 §5-1 이 ★로 둔 자리다. 계약의 `TerminalProcess.canPrintLabel` 을 진입 시 읽어
 ...
 * 준다(G-3).
 *
 * ⚠ **`P-02-05` 의 같은 파일을 사본으로 가져왔다 — 이것이 두 번째 사용처다**(스펙 §5-1 은
 * `P-02-07` 까지 셋으로 적는다). 그래도 아직 공용으로 올리지 않는다 — 화면 슬라이스를 사본으로
 * 소유하는 것이 이 저장소의 관례이고, 옮길 자리(`patterns/`)는 세 번째가 설 때 정한다.
 *
...
export const reprintGatingKeys = {
  /*
  ...
  processes: (terminalId: number, processId: number) =>
    ['packing-label-reprint', 'terminal-processes', terminalId, processId] as const,
};
...
export const useTerminalGate = (
  ...
  const query = useQuery({
    queryKey: reprintGatingKeys.processes(terminalId ?? 0, processId ?? 0),
    enabled: identified,
    queryFn: () => {
