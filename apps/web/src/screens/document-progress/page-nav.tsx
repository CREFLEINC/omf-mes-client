 ...
 * 규칙·현재 위치 표시·키보드 이동 규약을 갖는 별도의 부품이 되고, 되돌릴 문서를 찾는 데
 * 「7쪽으로 점프」는 정상 경로가 아니다 — 문서번호·기간·품목으로 좁히는 것이 정상 경로다.
 *
 * **전송 중 잠금이 아직 없다.** 이 회차에는 쓰기가 하나도 없어 잠글 것이 없다 —
 * 취소 조작이 생기는 회차에서 이 부품이 그 잠금을 받는다(쪽이 바뀌면 고른 문서가 풀리므로,
 * 전송 중에 열어 두면 앞 요청의 결과가 다른 맥락에 나타난다).
 *
