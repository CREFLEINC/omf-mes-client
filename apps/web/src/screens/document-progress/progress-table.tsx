 ...
 * | 상태 | 88px | 머리글 2자 15px + 여백 32px = 47px. ⚠ 아래 주 |
 * | 계획 수량 | 80px | 머리글 5자(공백 포함) 37.5px + 여백 32px = 69.5px. 수량 자릿수 여유 포함. 우측 정렬 |
 * | 처리 수량 | 80px | 같은 위 |
 * | 잔여 수량 | 80px | 같은 위 |
 ...
 * 접히지 않는 하한**(세부구분 62px · 상태 47px)에 짧은 코드 몫을 더한 값이고 그보다 긴 코드는
 * 접힌다. 지어낸 상한으로 「보장한다」고 적지 않는다. **두 열의 폭을 같은 88px로 두는 이유는
 * 머리글이 아니라 그 코드 몫이다** — 나란히 선 두 코드 칸의 폭이 다르면 값의 길이가 아니라
 * 머리글의 길이가 칸의 크기를 정한 것처럼 읽힌다.
 *
