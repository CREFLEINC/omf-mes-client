...
export const StocktakingScreen = () => {
   ...
   * | :-: | --- | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: |
   * | 1 | 조건 변경·조회 | 바뀐다 | **첫 쪽** | **비운다** | **비운다** | **비운다** | 유지 | **비운다** | 비운다 | **닫는다** | 개시 유지 · **치환·마감 비운다** | **비운다** |
   * | 2 | 초기화 | **비운다** | 첫 쪽 | 비운다 | 비운다 | **비운다** | 유지 | 비운다 | 비운다 | **닫는다** | 개시 유지 · **치환·마감 비운다** | 비운다 |
   * | 3 | 쪽 이동 | 유지 | 옮긴 쪽 | **비운다** | **비운다** | **비운다** | 유지 | **비운다** | 비운다 | **닫는다** | 개시 유지 · **치환·마감 비운다** | 비운다 |
   * | 4 | 실사 고르기·해제 | 유지 | **유지** | 넣고 뺀다 | **비운다** | **비운다** | 유지 | **비운다** | 비운다 | **닫는다** | 개시 유지 · **치환·마감 비운다** | **비운다** |
   * | 5 | 위치 고르기·해제 | 유지 | 유지 | 유지 | 넣고 뺀다 | 유지 | 유지 | **비운다** | 비운다 | **닫는다** | 개시·마감 유지 · **치환 비운다** | 유지 |
   * | 6 | **상세가 404** | 유지 | 유지 | **비운다** | **비운다** | **세운다** | 유지 | 비운다 | 비운다 | **닫는다** | 개시 유지 · **치환·마감 비운다** | 비운다 |
   ...
   * - **「서버 실패」 열의 한 규칙**(리뷰 R-1 · 계획 정정 5): **배너는 자기 대상보다 오래 살지
   *   않는다.** 실패는 「그 초안에 대한 사실」이므로 **그 초안이 매인 것**이 대상이다 —
   *   개시 초안은 **만들 실사**에 매여 있어 위에서 무엇을 고르든 살아남고(1~5행), 라인 초안은
   *   **고른 위치**에, 마감은 **고른 실사**에 매인다. 그래서 셋의 수명이 열 하나 안에서 갈린다.
   * - **1~6행이 「개시 유지」인 이유**: 개시 초안이 그 조작들에 살아남으므로(1~5행) 초안은
   *   남는데 왜 거절됐는지만 사라지면 사용자는 같은 값을 그대로 다시 보낸다.
   * - **마감 실패를 비우는 이유**(R-1): 그 근거가 **초안이 있는 쓰기의 것**이라 마감에는
   *   성립하지 않는다 — 마감에는 초안이 없다(보내는 값이 영업일 하나이고 화면이 파생한다).
   *   남으면 실사 A의 「이미 마감된 실사입니다」가 **실사 B의 상세 아래**에 선 채 B의 마감
   *   버튼은 활성이 된다. 5행에서 유지하는 것은 **같은 실사**이기 때문이다.
   * - **5행에서 치환 실패를 비우는 이유**(정정 5): **그 행이 라인 초안을 비우는 행**이라 유지
   *   근거가 거꾸로 선다 — 초안이 사라진 자리에 앞 위치의 거절 사유만 남으면 위치 A의
   *   「권한이 없습니다」가 **위치 B의 빈 표 위**에 서고 사용자는 B도 막힌 것으로 읽는다.
   *   1~4·6행에서는 `loc`가 비어 구획째 그려지지 않지만 **감춰지는 것과 지워지는 것은 다르다** —
   *   감춰진 실패는 그 위치를 다시 고르는 순간 되붙는다.
   */
  /*
  ...
  /*
   * **치환 실패는 고른 위치가 바뀌면 거둔다**(수명 표 1~5행 · 계획 정정 5 · 마감의 R-1과 동형).
   *
   * 「서버 실패 = 유지」의 근거는 「**초안은 남는데** 그 초안이 왜 거절됐는지만 사라지면 사용자는
   * 같은 값을 그대로 다시 보낸다」인데, **5행은 라인 초안을 비우는 행이다** — 초안이 사라진
   * 자리에 앞 위치의 거절 사유만 남으면 그 근거가 거꾸로 선다. 위치 A의 「권한이 없습니다」가
   * **위치 B의 빈 표 위에** 서면 사용자는 B도 막힌 것으로 읽는다.
   *
   * 1~4·6행은 이 자리 없이도 배너가 사라진다(`loc`가 비면 구획째 그려지지 않는다) — 그러나
   * **지워지는 것과 감춰지는 것은 다르다**: 감춰진 실패는 그 위치를 다시 고르는 순간 되붙는다.
   *
   * **바로 위 effect와 갈라 둔다.** 그쪽의 「의존성이 `loc` 하나뿐」은 #43을 막는 규칙이라
   * (감지기 M35) 다른 일을 얹으면 그 규칙이 무엇을 지키는지 읽히지 않는다. **의존성은 여기서도
   * 고른 위치 하나다** — `replace.reset`은 렌더마다 새 참조라 넣으면 매 렌더 배너가 지워진다.
   */
  useEffect(() => {
  ...
  /*
   * **마감 실패는 고른 실사가 바뀌면 거둔다**(수명 표 1~4·6행 · 리뷰 R-1).
   *
   * 「서버 실패 = 유지」의 근거는 **초안이 있는 쓰기**의 것이었다 — 「초안은 남는데 그 초안이
   * 왜 거절됐는지만 사라지면 사용자는 같은 값을 그대로 다시 보낸다」. **마감에는 초안이 없다**
   * (보내는 값이 영업일 하나이고 화면이 파생한다 — 그래서 취소 버튼도 두지 않았다).
   * 남길 이유가 없는데 남으면, 실사 A의 「이미 마감된 실사입니다」가 **실사 B의 상세 아래에
   * 선 채 B의 마감 버튼은 활성**이 된다 — 한 화면이 서로 어긋나는 두 말을 동시에 한다.
   *
   * 나머지 둘은 구획이 대신 거둔다: 개시 배너는 고른 실사가 없을 때만 보이는 구획에 있고,
   * 치환 배너는 `loc`가 비면 구획째 사라진다. **마감 배너만 자기 대상보다 오래 산다.**
   *
   * **의존성은 고른 실사 하나다.** `close.reset`은 렌더마다 새 참조라(공통 훅이 `useMutation`
   * 결과를 물고 있다) 의존성에 넣으면 **매 렌더 배너가 지워져** 실패가 보이지 않는다.
   * 위치만 바뀌는 조작에는 반응하지 않는다 — 실패는 **그 실사**에 대한 사실이다(5행).
   */
  useEffect(() => {
