 ...
 * 이 화면이 주소에서 읽는 값.
 *
 * **주소는 설비를 «덮어쓰는» 자리다.** POP 단말은 설비에 붙어 있고(스펙 §4-A — 헤더에서
 * 고정), 그 붙임은 이제 셸이 알려 준다(`patterns/pop-identity` 의 `equipment` · #1149).
 * 화면의 기본 출처는 단말이고, 주소는 단말이 아닌 설비를 일부러 볼 때(개발 이동표·점검)만
 * 쓰인다 — 읽는 방식은 여기 한 자리로 남는다.
 *
