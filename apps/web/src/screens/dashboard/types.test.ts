...
describe('formatAsOf', () => {
  it('서버가 준 순간을 공장 시각(UTC+7)으로 낸다 — 보는 사람의 시간대와 무관하다(omf-all-around#20)', () => {
    expect(formatAsOf('2026-08-24T09:12:00+09:00')).toBe('2026-08-24 07:12');
  });

  it('UTC(Z)로 온 값도 공장 시각으로 바꾼다 — 같은 순간이면 offset이 달라도 표기가 같다', () => {
    expect(formatAsOf('2026-08-24T09:12:00Z')).toBe('2026-08-24 16:12');
    expect(formatAsOf('2026-08-24T00:12:00Z')).toBe(formatAsOf('2026-08-24T09:12:00+09:00'));
  });
...
describe('toDashboardView', () => {
  ...
  it('카드의 빈 칸을 0이 아니라 null로 눕힌다', () => {
    const view = toDashboardView(
      summary({
        cards: [
          { cardCode: 'OEE', label: '합성 지표 가', value: 12, valueStatusCode: 'AVAILABLE' },
        ],
      }),
    );
