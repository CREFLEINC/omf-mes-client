...
/**
 * 판정유형 코드 그룹을 찾기 위한 조회 조건. 값이 바뀔 일이 없어 **모듈 수준 상수**로 둔다.
 *
 * ⚠ **이 상수가 「조회를 한 번만 나가게」 하는 것은 아니다.** 조회 열쇠는 참조가 아니라 **내용으로
 * 해시**되고, 열쇠 배열 자체도 `code-group-queries.ts`가 렌더마다 새로 만든다 — 이 객체를 렌더
 * 안에서 만들어도 요청 횟수는 같다. **「참조를 고정하면 요청이 준다」로 읽지 말 것.**
 *
 * 그래도 상수로 두는 이유는 매 렌더 다시 만들 이유가 없어서이고, 이 저장소의 다른 화면도 같은
 * 자리를 같은 모양으로 둔다. 다만 **이 선택을 지키는 테스트는 없다** — 코드 확인으로만 지켜진다.
 *
