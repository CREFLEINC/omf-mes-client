...
export interface PartnerRoleOption {
  ...
  label: string;
  /**
   * 이 화면이 아는 어휘인가. **아니라고 해서 잘못된 값이 아니다** —
   * 계약이 값 목록을 아직 못 박지 않았다. 아니면 「이 화면이 모르는 역할」 표식이 붙는다.
   */
  isKnown: boolean;
}
...
const VOCABULARY_ROLES: readonly PartnerRole[] = PARTNER_ROLE_ORDER.map((key) => ({
  roleTypeCode: PARTNER_ROLE_CODES[key],
}));

/** 앞에 온 것을 남기고 같은 코드를 접는다. 옛 자료가 같은 짝을 두 번 줄 수 있다. */
const dedupeByCode = (roles: readonly PartnerRole[]): PartnerRole[] => {
 ...
 */
export const toPartnerRoleDraft = (roles: readonly PartnerRole[]): string[] =>
 ...
 */
export const toPartnerRoleOptions = (baseline: readonly PartnerRole[]): PartnerRoleOption[] =>
...
/**
 * 저장하면 **해제되는** 역할 — 붙어 있었는데 체크가 꺼진 것.
 *
 * **어휘 밖 코드도 든다**(결정 8). 빠뜨리면 화면이 모르는 역할이 확인 창에도 나오지 않은 채
 * 사라지고, 사용자는 자기가 무엇을 잃었는지 알 방법이 없다.
 *
...
export const releasedPartnerRoles = (
  ...
  return toPartnerRoleOptions(baseline).filter(
    (option) => attached.has(option.roleTypeCode) && !chosen.has(option.roleTypeCode),
  );
};
 ...
 *
 * 차례는 **체크칸 목록이 준 차례**(어휘 다섯 다음 어휘 밖)다. 집합이라 차례에 뜻은 없지만
