...
export const DepartmentFormPane = ({
   ...
   *
   * 형제 구획(코드그룹)과 같은 갈래 순서이며, **자격 구획의 셋째 갈래(중복 짝 차단)에
   * 해당하는 사정이 이 구획에도 없다** — 서버가 거부할 것을 **미리 아는** 자리가 없기
   * 때문이다. 자격은 렌더 시점에 초안끼리 견줘 중복을 판정하지만(그 판정기는 자격 전용이다),
   * 부서 검증은 저장을 누른 뒤 상위에서 돈다. 갈래가 넷에서 셋으로 준 이유가 이것이다 —
   * 다음 사본이 「셋이 원래 그런 것」으로 받지 않도록 근거를 여기 남긴다.
   */
