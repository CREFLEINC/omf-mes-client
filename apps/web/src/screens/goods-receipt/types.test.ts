...
describe('formatDateTime', () => {
  it('날짜와 분까지만 공장 시각(UTC+7)으로 보인다', () => {
    expect(formatDateTime('2026-08-06T09:12:00+09:00')).toBe('2026-08-06 07:12');
  });

  /*
   * **공장 시각(베트남, UTC+7)으로 옮긴다(omf-all-around#20).** 같은 순간이면 offset이 달라도
   * 같은 시각을 낸다 — 보는 사람의 실행 환경 시간대와 상관없이 같은 전표는 같은 시각으로 보인다.
   */
  it.each(['2026-08-06T11:12:00+09:00', '2026-08-06T02:12:00Z', '2026-08-05T21:12:00-05:00'])(
    '같은 순간이면 offset이 달라도(%s) 같은 공장 시각을 낸다',
    (value) => {
      expect(formatDateTime(value)).toBe('2026-08-06 09:12');
    },
  );

  it.each([
    ['2026-08-06T09:12:00Z', '2026-08-06 16:12'],
    ['2026-08-06T09:12:00-05:00', '2026-08-06 21:12'],
    ['2026-08-06T18:30:00Z', '2026-08-07 01:30'],
  ])('다른 순간(%s)이면 다른 시각(%s)을 낸다 — 자정을 넘으면 날짜도 바뀐다', (value, expected) => {
