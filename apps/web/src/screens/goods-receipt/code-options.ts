 ...
 *
 * ⚠ **재고 상태는 그 사정이 한 번 바뀌었다.** 계약이 값 넷(`AVAILABLE`·`IN_TRANSIT`·
 * `ON_HOLD`·`BLOCKED`)을 실제로 enum으로 못박았다(재동기화 #124·#128). 그래도 **여기는 여전히
 * 비어 있다** — 그 근거의 문면이 「1차 제안안」이고, 채우는 순간 이 화면의 등록이 열려
 * 사용자 흐름이 바뀌기 때문이다. 확정 회신이 오면 그때 여는 것이 별도의 결정이다.
 * 값 넷의 목록은 `gr-request.ts`가 생성물 타입에서 파생해 들고 있고, 「좁힌 뒤에도 비어 있다」는
