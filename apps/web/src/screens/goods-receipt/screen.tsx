...
export const GoodsReceiptScreen = () => {
  ...
  const postPane = (inboundReceipt: IrView, line: IrLineView): ReactNode => {
    ...
    const submit = (): void => {
      ...
      /*
       * 조립이 마지막으로 거른다 — 주입된 상태나 복원된 화면 상태가 계약 enum 밖이면
       * 요청을 내보내지 않는 마지막 방어선이다.
       *
       * **여기서 되돌아가면 사용자는 아무 되먹임도 받지 못한다** — 결과 구획은 위에서 비웠고,
       * 훅이 아무 실패도 받지 않아 실패 문구도 서지 않는다. 현재 선택지는 OpenAPI enum만
       * 내지만, 외부에서 주입되거나 복원된 비정상 값이 이 겹을 우회하지 못하게 유지한다.
       */
      if (body === null) return;
