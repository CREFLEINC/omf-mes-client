    ...
    async (targets: readonly PrintTarget[]): Promise<void> => {
      ...
      const outcome: PrintOutcome = await printAll(targets, {
          ...
          const data = await runRequest<ArrayBuffer>(() =>
            client.GET('/app/document-issues/{documentIssueLogId}/rendition', {
              params: {
                ...
                query: { format: toContractFormat(LOT_LABEL_RENDITION_FORMAT) },
              },
              parseAs: 'arrayBuffer',
            }),
          );
        ...
        send: async (bytes, label) => {
          await shell.save(bytes, label, new Date().toISOString(), LOT_LABEL_RENDITION_FORMAT);
        },
      ...
      setState(
        outcome.ok
          ? { phase: 'succeeded', printed: outcome.printed, reason: null }
          : { phase: 'failed', printed: outcome.printed, reason: outcome.reason },
      );
    },
