 ...
 * ```
 *
 * ⛔ **발행과 인쇄를 한 호출로 묶지 않는다.** 스펙 §6 이 「프린터 오프라인 — 기록은 남고
 * 인쇄만 실패」로 못박았다. 그 사실이 곧 재출력으로 복구할 수 있다는 뜻이다.
 *
 * ⛔ **화면이 라벨을 그리지 않는다.** 서버가 그린 바이트를 그대로 넘긴다(결정 18 · K-5) —
 * 단말마다 출력물이 달라지지 않게 하기 위함이다. 라벨 서식이 미정인 것(스펙 §8-2)이 이 화면을
 * 막지 않는 이유도 같다.
 *
 ...
 * 수 없다」는 사실이라, 화면은 그 사유를 말하고 발행 전부터 조작을 막는다.
...
export interface PrintTarget {
  ...
  /** 인쇄 작업 이름. 현장에서 어느 출력물인지 가리는 값이라 **LOT 번호**를 쓴다. */
  label: string;
}
 ...
 *
 * ⭐ **첫 실패에서 멈춘다.** 프린터가 죽은 상태로 계속 밀면 그만큼의 실패 보고가 나가고
 * 사용자는 그 사이 아무것도 할 수 없다. 멈춘 뒤 남은 것은 회차가 오르지 않은 채 그대로 있고,
 * 재출력이 이어받는다.
 *
