...
/** 고르지 않은 상태. ⛔ 값이 빈 글자여야 조회 조건에서 빠진다. */
const ALL_OPTION = { value: '', label: t.all };

/**
 * 선택지를 세울 수 없을 때의 사유. **못 받았을 때만** 끈다.
 *
 * ⛔ **받는 중에는 끄지 않는다.** 끄면 사유를 적을 것이 없는데(잠깐 뒤면 채워진다) G-2 는
 * 「비활성에는 사유를 함께」를 요구한다 — 사유 없는 비활성이 그래서 생긴다. 받는 중에는
 * 「전체」만 있는 채로 열어 두고, 값이 오면 늘어난다.
 */
const lookupNote = (lookup: NameLookup): string | null => {
  if (lookup.isError) return t.lookupFailed;

  /* ⛔ 「여기 없으면 없는 것」으로 읽히지 않게 한다 — 앞의 몇 건만 받아 둔 목록이다. */
  return lookup.isTruncated ? t.optionsTruncated(LOOKUP_SIZE) : null;
};
...
export const ProgressFilterBar = ({
  ...
  const lineNote = lookupNote(lineLookup);
  const orderNote = lookupNote(productionOrderLookup);
  const statusReason = statusOptions.isUnavailable ? t.statusUnavailable : null;

  const periodNoteId = `${fromId}-note`;
  const lineNoteId = `${lineId}-note`;
  const orderNoteId = `${orderId}-note`;
  ...
  return (
    <form aria-label={t.legend} className="filter-bar" onSubmit={submit}>
      ...
      <div className="field-cell">
        <label className="field-label" htmlFor={lineId}>
          {t.productionLine}
        </label>
        <Select
          aria-describedby={lineNote === null ? undefined : lineNoteId}
          disabled={lineLookup.isError}
          id={lineId}
          options={[ALL_OPTION, ...lineLookup.options]}
      ...
      <div className="field-cell">
        ...
        <Select
          ...
          id={statusId}
          options={[ALL_OPTION, ...statusOptions.options]}
      ...
      <div className="field-cell">
        ...
        <Select
          aria-describedby={orderNote === null ? undefined : orderNoteId}
          disabled={productionOrderLookup.isError}
          id={orderId}
          options={[ALL_OPTION, ...productionOrderLookup.options]}
        ...
        />
        {orderNote === null ? null : (
          <p className="field-note" id={orderNoteId}>
            {orderNote}
          </p>
        )}
      </div>
