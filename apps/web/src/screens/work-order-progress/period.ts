 ...
 * 진행현황 조회의 기간.
 *
 * 이 화면이 거는 두 파라미터(`plannedStartFrom`·`plannedStartTo`)는 계약에서 **`date-time`**
 * 이다(실측). 그래서 두 규약이 함께 걸린다.
 *
 ...
 *
 * ⛔ **넓은 기간은 막지 않고 «경고»만 한다**(스펙 §6). 그래서 이 화면의 판정에는 앞선 화면에
 * 없던 갈래가 하나 더 있다 — 「통과했는데 할 말이 있는」 상태다.
 *
...
/** 이 날 수를 넘으면 경고한다 — 막지는 않는다. 3개월을 날로 센 값이다. */
export const WIDE_PERIOD_DAYS = 92;
 ...
 *
 * `warning`은 **통과한 뒤에 붙는 말**이다. 막는 사유(`blocked`)와 자리를 나눠 둔 이유는,
 * 한자리에 두면 「경고인데 막힌」·「막혔는데 조회되는」 상태가 생기기 때문이다.
 */
export type PeriodState =
  | { kind: 'ready'; bounds: PeriodBounds; warning: string | null }
 ...
 * 대상 지역(한국 UTC+9 · 베트남 UTC+7)에 서머타임이 없어 성립한다.
 */
...
/** 두 날 사이의 날 수 — **양 끝을 포함해** 센다. 화면이 「며칠치를 본다」로 읽는 값이다. */
const spanInDays = (from: CalendarDate, to: CalendarDate): number => {
 ...
 *
 * ⭐ **막는 사유를 세 갈래로 가른다** — 셋의 해법이 서로 다르기 때문이다(G-3: 비활성 사유는
 * 「왜」가 아니라 「어떻게 풀 것인가」를 담는다). 순서도 뜻이 있다: 달력에 없는 날을 「두 날짜를
 * 바꾸세요」로 안내하면 바꿔도 풀리지 않는다.
 */
export const resolvePeriod = (input: PeriodInput, offsetMinutes: number): PeriodState => {
  if (input.from === '' || input.to === '') {
    return { kind: 'blocked', reason: t.periodRequired };
  }
  ...
  if (from === null || to === null) {
    return { kind: 'blocked', reason: t.periodInvalid };
  }

  // 자릿수가 고정된 `YYYY-MM-DD`라 문자열 비교가 곧 날짜 비교다.
  if (input.to < input.from) {
    return { kind: 'blocked', reason: t.periodReversed };
  }
  ...
  return {
    ...
    warning: spanInDays(from, to) > WIDE_PERIOD_DAYS ? t.periodWide : null,
  };
};
