...
describe('readFilters — 주소가 조건의 정본이다', () => {
  it('주소 키를 조건으로 옮긴다', () => {
    expect(
      readFilters(params('type=SAMPLE_TARGET_A&target=9101&event=SAMPLE_EVENT_A&by=9201&corr=C-1')),
    ).toEqual({
      targetType: 'SAMPLE_TARGET_A',
      targetId: '9101',
...
describe('toSearchParams — 빈 조건은 키 자체를 두지 않는다', () => {
  it('기간과 걸린 조건만 적는다', () => {
    const next = toSearchParams(
      ...
      filters({ targetType: 'SAMPLE_TARGET_A' }),
      1,
    );

    expect(next.toString()).toBe('from=2026-08-01&to=2026-08-07&type=SAMPLE_TARGET_A');
  });
...
describe('toFilterQuery — 계약 쿼리 이름으로 옮긴다', () => {
  it('걸린 조건만 쿼리에 담고 번호는 숫자로 보낸다', () => {
    expect(
      ...
      targetTypeCode: 'SAMPLE_TARGET_A',
      targetId: 9101,
...
describe('toFilterChips', () => {
  it('적용된 조건마다 하나씩 나오고 순서는 조건 줄과 같다', () => {
    const chips = toFilterChips(
      filters({ targetType: 'SAMPLE_TARGET_A', performedBy: '9201', correlationId: 'C-1' }),
    );

    expect(chips.map((chip) => chip.key)).toEqual(['targetType', 'performedBy', 'correlationId']);
    expect(chips[0]?.label).toBe('대상 종류: SAMPLE_TARGET_A');
  });
