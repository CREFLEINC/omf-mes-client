...
describe('readFilters', () => {
  ...
  it('세 조건을 읽는다', () => {
    expect(readFilters(paramsOf('it=1001&sp=2002&q=IR-2026'))).toEqual({
      itemId: 1001,
      supplierId: 2002,
      keyword: 'IR-2026',
    });
...
describe('toListQuery', () => {
  ...
  it('채운 조건만 싣는다', () => {
    expect(toListQuery({ itemId: 7, supplierId: null, keyword: '' }, 2)).toEqual({
      inspectionTypeCode: 'IQC',
      pendingOnly: true,
      itemId: 7,
      page: 2,
  ...
  it('공급사를 식별자로 싣는다 — 조인은 서버가 푼다', () => {
    expect(toListQuery({ ...EMPTY_FILTERS, supplierId: 2002 }, 1).supplierId).toBe(2002);
  });
...
describe('toSearchParams', () => {
  ...
  it('고른 의뢰를 싣지 않는다 — 조건이 바뀌면 그 의뢰가 목록에서 사라질 수 있다', () => {
    const params = toSearchParams({ ...EMPTY_FILTERS, itemId: 9 });

    expect(params.has('ir')).toBe(false);
  });

  it('채운 조건을 적는다', () => {
    const params = toSearchParams({ itemId: 12, supplierId: 34, keyword: 'IR' });

    expect(Object.fromEntries(params)).toEqual({ it: '12', sp: '34', q: 'IR' });
  });

  it('고정 축을 주소에 적지 않는다 — 지울 수 없는 것을 조건처럼 보이면 안 된다', () => {
    const params = toSearchParams({ itemId: 12, supplierId: null, keyword: '' });
...
describe('toPageParams', () => {
  it('조건과 고른 의뢰를 그대로 둔 채 쪽만 옮긴다 — 조건이 안 바뀌었으니 선택을 버릴 이유가 없다', () => {
    const next = toPageParams(paramsOf('it=7&ir=1001'), 3);

    expect(Object.fromEntries(next)).toEqual({ it: '7', ir: '1001', page: '3' });
  });

  it('첫 쪽으로 돌아가면 쪽 키를 지운다', () => {
    const next = toPageParams(paramsOf('it=7&page=5'), 1);

    expect(next.has('page')).toBe(false);
    expect(next.get('it')).toBe('7');
  });
});
