...
describe('RuleListPane — 표 규율', () => {
  ...
  /**
   * **선언 쪽을 리터럴로 못 박는다.** 바로 위 감지기는 상수를 상수와 견주고 있어 선언 절반이
   * 비어 있다 — 폭을 지정한 열이 **몇인지**는 아무도 재지 않는다.
   *
   * ⛔ 「산출물 `<col>` 수와 열 머리 수가 같다」로는 이 자리를 재지 못한다. 둘 다 디자인 시스템
   * 표가 **같은 `columns` 배열에서** 하나씩 만들어 내므로(폭을 주지 않은 흡수 열도 `<col>`을
   * 갖는다) 두 수는 **정의상 언제나 같고**, 열을 더하든 빼든 울지 않는다 — 재는 것이 화면의
   * 선언이 아니라 DS의 내부 정합성이다.
   *
   * 열 증감은 이 단언과 **짝**인 「흡수 열은 이름 열 둘뿐이다」가 함께 문다 —
   * 폭을 주고 열을 더하면 이 단언이, 폭 없이 더하면 짝이 운다.
   */
  it('폭을 지정하는 열은 넷이다', () => {
    expect(Object.keys(RULE_COLUMN_WIDTH)).toHaveLength(4);
  });
