 ...
 *
 * 데이터에 유일 제약이 없어 **계약이 400으로 막는 조합**이 있다. 이미 들어와 있는 중복은 화면이
 * 먼저 보여야 한다: 그 상태에서는 어느 규칙이 이기는지 데이터가 정하지 않으므로 현장 적치가
 ...
 * 공유계약 B-12와 이슈 §6은 4키를 말한다.
 *
 * 그래서 **4키를 기본값으로 채택하고 착수 코멘트로 질의했다**(승인 기록 부록 B Q2). 확정 사실이
 * 아니다. 3키가 정답으로 오면 「우선순위만 다른 두 규칙」을 서버는 막는데 화면은 표시하지 않으므로
 * **여기 비교 키에서 우선순위를 빼야 한다.** 단위 ③의 조준 조회가 같은 근거를 물려받는다.
 *
 * ## 이 판정이 말하는 범위
 *
 * 화면은 현재 쪽이 아니라 창고의 활성 규칙 전건을 넘긴다. 따라서 다른 쪽의 상대 때문에 생긴
 * 중복도 현재 행에 표시된다. 저장 차단은 같은 네 축을 쓰는 조준 조회가 맡는다.
 *
 * ## 꺼진 규칙은 세지 않는다
 *
