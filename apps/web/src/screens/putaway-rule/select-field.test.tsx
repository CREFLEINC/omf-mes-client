 ...
 *
 * T1이 인계에서 지목한 자리이며(「잠긴 사유 우선순위 감지기」) 등록·수정 폼이 서면서
 * 셋이 처음으로 한 칸에 모였다.
 */
const renderField = (overrides: Partial<SelectFieldProps> = {}) => {
  ...
  render(
    <SelectField
      label="단위"
      options={[{ value: '9401', label: 'SYN-UOM-01 · 합성단위 가' }]}
      value=""
      onChange={onChange}
      {...overrides}
    />,
...
describe('SelectField — 보조 문구의 우선순위', () => {
  ...
  it('오류가 목록 안내를 밀어낸다', () => {
    renderField({ error: '단위를 고르세요.', note: '선택지가 앞쪽 일부만 보입니다.' });

    expect(screen.getByText('단위를 고르세요.')).toBeInTheDocument();
  ...
  it('잠긴 칸에 오류가 함께 있으면 오류가 선다', () => {
    renderField({ error: '단위를 고르세요.', disabled: true, disabledReason: '저장이 끝난 뒤에' });
  ...
  it('보조 문구가 컨트롤의 설명으로 이어진다', () => {
    renderField({ error: '단위를 고르세요.' });

    const describedBy = screen
      .getByRole('combobox', { name: '단위' })
      .getAttribute('aria-describedby');

    expect(describedBy).not.toBeNull();
    expect(document.getElementById(describedBy ?? '')).toHaveTextContent('단위를 고르세요.');
  });
});
