 ...
 *
 * **배치 클래스(`className`)를 받지 않는다.** 전례에는 있으나 이 슬라이스의 두 소비처
 * (저장·취소)가 액션 줄에서 자리를 따로 정하지 않는다 — 값만 안 넘기고 인자를 남기면
 * 「이 슬라이스에 그 자리가 없다」가 타입 수준의 사실이 되지 못한다(사본 체크리스트 7번).
 * 이차 액션이 서는 회차(끄기·켜기)가 그때 필요한 형태로 더한다.
 *
