 ...
 *
 * ## ⚠ 옆의 `judgeRemainingCoverage`와 **일부러 다르게** 다룬다
 *
 * 같은 조준 조회를 읽는 두 판정인데, 조회 축 밖의 행(창고·품목이 어긋난 행)이 오면 그쪽은
 * 판정을 **멈추고**(`outOfScope`) 이쪽은 **키로 걸러 센다**. **한쪽만 고친 것이 아니다** —
 * 위험의 방향이 반대다: 그쪽은 틀리면 「이것뿐입니다 · 검증 없이 통과합니다」라는 **거짓을
 * 단언**하고, 이쪽은 틀려도 **막지 않는 쪽**으로 실패해 계약의 400이 뒤를 받는다. 이쪽을
 * 그쪽에 맞추면 **조회 실패 하나가 마스터 관리를 멈춘다**(`activation-guard.ts`의 같은 절).
 *
