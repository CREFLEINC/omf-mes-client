 ...
 *
 * **응답에 `ETag`가 없다**(계약 실측). 그래서 성공 뒤 **상세 키를 무효화한다** — 다만 이 사본에서
 * **낡은 토큰을 실제로 막는 것은 위 fetcher의 `staleTime: 0`**이다(리뷰 R-26 실측). 이 키를
 * 구독하는 구획이 이 화면에 없어(`fetchQuery`로만 부른다) 무효화가 재조회를 일으키지 않기
 * 때문이다. 무효화를 남겨 두는 이유는 **캐시에 남은 값이 상신 전의 것**이라, 이 키를 그리는
 * 구획이 생기는 날 낡은 값을 그대로 그리지 않게 하려는 것이다 — 두 줄은 서로를 대신하지 못한다.
 * **입하 전표는 무효화하지 않는다**: 그 응답을 다시 받으면 승계 초안이 다시 서고(화면의 수명 표)
 * 사용자가 보고 있던 값이 갈린다.
 *
