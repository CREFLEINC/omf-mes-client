...
export interface SubmitSummary {
  ...
  /** 요청 사유 **전문**. 줄바꿈이 유지된다 */
  reason: string;
  /** 결재함 목록에서 요약을 겸할 첫 줄. **전문과 나눠 보인다**(계획 결정 15) */
...
/**
 * 상신 확인 — **되돌릴 수 없는 둘째 조작 앞의 마지막 층이다.**
 *
 * **등록과 별개 동작이다**(착수 이슈 §6 ③ · 계획 결정 9). 이 창은 **이미 만들어진** 전표를
 * 결재에 올리는 것만 확인한다 — 주 사본이 된 전례는 한 확인이 등록+상신 두 요청을 이었는데,
 * 그 형태를 베끼면 이 화면이 착수 이슈를 어긴다.
 *
 * **사유 전문과 첫 줄을 나눠 보인다**(계획 결정 15). 결재함 목록에서 요약을 겸하는 것이 첫
 * 줄이라(공유계약 A-12) 어느 문장이 남들에게 보일지 보내기 전에 확인시킨다.
 *
 * **승인자·결재선을 이 창이 고르지 않는다**(착수 이슈 §6 ④ · 계획 결정 8). 계약의 본문이 사유
 * 한 칸이라 보낼 자리가 없고, 승인 주체는 결재선 정의(W-06-15)가 정한 대로 전개된다 —
 * 그 사실을 적어야 사용자가 「고르는 칸이 없다」를 결함으로 읽지 않는다.
 *
 ...
 * 시스템이 막을 수단을 주지 않으므로, 그 길로 닫혀도 무너지지 않게 화면이 **나가는 중인
 * 쓰기를 끊지 않는** 규율을 진다.
 *
