...
describe('DiscardConfirmDialog — 닫히는 것이 곧 버리는 것은 아니다', () => {
   ...
   *
   * 이 저장소의 버리기 창 **여섯이 모두** 스크림을 열어 두고, **그중 넷이** 스크림 클릭
   * 감지기를 실으며, **`goods-receipt` 하나**가 「닫힘 = 버리지 않음」의 짝까지 고정한다
   * (실측 — 나머지 셋은 `onClose` 한 줄뿐이라 「닫힌다」만 잰다).
   * 그 규율을 일곱 번째 화면에서 뒤집으면 규칙이 「막는다」로 굳어, 정작 **되돌릴 수 없는 창을
   * 막는 이유**(등록 확인 창)와 구분되지 않는다.
   *
   * 이 화면은 스크림과 X **양쪽 모두** 짝을 고정한다 — 그 형태는 저장소에서 여기가 처음이다.
   *
