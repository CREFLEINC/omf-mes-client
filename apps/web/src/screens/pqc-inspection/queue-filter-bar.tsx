...
/**
 * 좌측 검사 대기 큐의 조건 줄 — **작업지시·생산 LOT·의뢰번호 셋뿐이다.**
 *
...
export const QueueFilterBar = ({ appliedFilters, onSearch, onReset }: QueueFilterBarProps) => {
   ...
   */
  const { workOrderId, lotId, keyword } = appliedFilters;

  useEffect(() => {
    setDraft(toDraft({ workOrderId, lotId, keyword }));
    setShowErrors(false);
  }, [workOrderId, lotId, keyword]);
  ...
  return (
    <form className="filter-bar" onSubmit={submit}>
      <TextField
        ...
        error={errorOf(errors.workOrder)}
        onChange={(event) => setDraft({ ...draft, workOrder: event.target.value })}
      />
      <TextField
        label={t.lot}
