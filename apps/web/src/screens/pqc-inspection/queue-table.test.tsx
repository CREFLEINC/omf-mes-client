...
describe('QueueTable', () => {
  ...
  it('작업지시가 없으면 빈 칸이 아니라 없음 표시를 낸다 — 못 불러온 것과 구분한다', () => {
    renderTable();
  ...
  it('대기와 진행을 서로 다른 문구로 가른다', () => {
    ...
    expect(
      within(waiting as HTMLElement).getByText(messages.pqcInspection.status.requested),
    ).toBeInTheDocument();
    expect(
      within(inProgress as HTMLElement).getByText(messages.pqcInspection.status.inProgress),
    ).toBeInTheDocument();
  });
