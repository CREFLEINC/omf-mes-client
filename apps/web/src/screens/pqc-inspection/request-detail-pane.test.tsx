...
import { requestWithoutWorkOrder, waitingRequest } from './fixtures';
...
describe('RequestDetailPane', () => {
  ...
  it('자재 LOT 이 없으면 빈 칸이 아니라 없음 표시를 낸다', () => {
    renderWithProviders(
      <RequestDetailPane detail={toInspectionRequestDetail(requestWithoutWorkOrder)} />,
    );

    expect(screen.getByText(messages.pqcInspection.queue.emptyValue)).toBeInTheDocument();
  });
});
