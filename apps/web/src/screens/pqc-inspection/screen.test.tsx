...
const renderScreen = (
  ...
  const view = renderWithProviders(<PqcInspectionScreen />, {
    route,
...
describe('PqcInspectionScreen', () => {
  ...
  it('고정 축을 늘 실어 보낸다 — 사용자가 끌 수 없는 이 화면의 정의다', async () => {
    ...
    expect(lastQuery(sent)?.get('inspectionTypeCode')).toBe('PQC');
    expect(lastQuery(sent)?.get('pendingOnly')).toBe('true');
  });

  it('주소가 담은 조건을 그대로 실어 보낸다 — 새로고침·공유가 같은 결과를 낸다', async () => {
    const { sent } = renderScreen('/?wo=1001&lot=2002&q=IR&page=2');

    await waitFor(() => expect(queueCalls(sent)).toHaveLength(1));

    expect(lastQuery(sent)?.get('workOrderId')).toBe('1001');
    expect(lastQuery(sent)?.get('lotId')).toBe('2002');
  ...
  it('조건을 바꾸면 첫 쪽부터 다시 부른다 — 좁힌 결과가 3쪽에 못 미칠 수 있다', async () => {
    ...
    await userEvent.type(screen.getByLabelText(t.filters.workOrder), '1001');
  ...
  it('쪽을 옮겨도 조건과 고른 의뢰는 그대로다', async () => {
    const { sent } = renderScreen('/?wo=1001&ir=1002', () =>
  ...
  /*
   * ⛔ **처분은 잠정이라 저장하지 않는다**(REQ-PR-0025 · 스펙 §5-8). 보내면 정본이 둘이 되고,
   * 뒤에 오는 확정이 이 잠정값과 어긋나도 어느 쪽이 옳은지 화면이 말할 수 없다.
   */
  it('고른 처분을 본문에 싣지 않는다 — 확정은 불량창고 입고 후다', async () => {
    const { writes } = renderScreen('/?ir=1002', () => jsonResponse(queueResponse()), []);
    ...
    const body = await bodyOf(writes[0] as Request);
    expect(body).not.toHaveProperty('dispositionTypeCode');
    expect(body).not.toHaveProperty('disposition');
  });

  /*
   * ⭐ **표본 검사는 대표 구간이 있어야 불합격 시 회수 범위가 정해진다**(§5-5). 비운 채로
   * 저장하면 그 근거가 영영 없다 — 비어 있으면 검사 시각으로 채워 보낸다.
   */
  ...
  /* ⭐ 사람이 고친 구간은 그대로 나간다 — 자동 채움이 사람의 판단을 덮지 않는다. */
  it('사람이 고친 생산구간을 그대로 보낸다', async () => {
    const { writes } = renderScreen('/?ir=1002', () => jsonResponse(queueResponse()), []);
    ...
    await userEvent.type(from, '2026-08-27T07:00:00+09:00');
    await userEvent.click(screen.getByRole('button', { name: t.result.save }));

    await waitFor(() => expect(writes).toHaveLength(1));

    expect((await bodyOf(writes[0] as Request)).coverageFromAt).toBe('2026-08-27T07:00:00+09:00');
  });
  ...
  it('검사 시점에 고정된 기준 버전으로 항목을 부른다 — 최신 기준을 따로 찾지 않는다', async () => {
    ...
    await screen.findByRole('region', { name: messages.pqcInspection.measurements.heading });
  ...
  it('아직 시작하지 않은 의뢰의 그리드가 「불러오는 중」에 머물지 않는다', async () => {
    ...
    expect(
      await screen.findByText(messages.pqcInspection.measurements.noItems),
    ).toBeInTheDocument();
    expect(screen.queryByText(messages.pqcInspection.measurements.loading)).not.toBeInTheDocument();
  });
  ...
  it('교정 만료로 잰 측정치가 있으면 경고를 세우되 막지는 않는다', async () => {
    ...
    expect(
      await screen.findByText(messages.pqcInspection.measurements.calibrationWarningTitle),
    ).toBeInTheDocument();
...
describe('PqcInspectionScreen — 재검사 회차', () => {
  /**
...
describe('PqcInspectionScreen — 확정 결과 문면', () => {
...
describe('PqcInspectionScreen — 재검사 저장 뒤', () => {
  /**
