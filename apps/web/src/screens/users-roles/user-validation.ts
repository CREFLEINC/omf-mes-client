 ...
 *
 * **길이 상한을 만들지 않는다** — 계약이 이 칸들에 길이 제약을 적어 두지 않았다.
 * 상한을 지어내면 서버가 받아 주는 값을 사용자가 넣지 못하게 되고, 그 막힘은 되돌릴 수단이 없다.
 * 서버가 길이로 거부하면 그 400이 해당 칸 옆 인라인으로 온다(둘 다 화면이 아는 필드다).
 *
...
export const validateUserForm = (
  ...
  // 계약이 널을 허용한다 — 비우는 것이 정상 값이라 형식은 값이 있을 때만 본다.
