...
/**
 * 단위 이름. 수량 뒤에 붙일 한 낱말만 필요해 이름표만 만든다.
 *
...
export const useUomLookup = (unknownLabel: string): UomLookup => {
  const { client } = useApiClient();

  const query = useQuery({
    queryKey: ['emergency-work-order-field', 'uoms'] as const,
    queryFn: async () => {
  ...
  return {
    labelOf: (uomId) => {
      if (uomId === undefined) return unknownLabel;

      return query.data?.get(uomId) ?? unknownLabel;
    },
  };
};
