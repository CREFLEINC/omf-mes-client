...
/**
 * ⭐ **공장 시각(베트남, UTC+7)으로 낸다**(omf-all-around#20). offset이 붙은 값은 그 순간을
 * 공장 시각으로 바꾼다 — 같은 순간이면 어느 offset으로 와도, 보는 사람의 시간대가 어디든
 * 같은 시각으로 보인다.
 */
describe('받은 시각 표기', () => {
  it('날짜와 시·분만 남긴다', () => {
  ...
  it('다른 offset으로 온 값은 그 순간의 공장 시각으로 낸다', () => {
    expect(formatObservedAt('2026-08-22T09:40:12+09:00')).toBe('2026-08-22 07:40');
    expect(formatObservedAt('2026-08-22T09:40:12Z')).toBe('2026-08-22 16:40');
  });

  it('같은 순간이면 offset이 달라도 표기가 같다', () => {
    expect(formatObservedAt('2026-08-22T11:40:12+09:00')).toBe('2026-08-22 09:40');
    expect(formatObservedAt('2026-08-22T02:40:12Z')).toBe('2026-08-22 09:40');
  });
