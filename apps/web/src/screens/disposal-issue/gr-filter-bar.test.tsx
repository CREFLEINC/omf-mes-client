...
const baseProps = (overrides: Partial<GrFilterBarProps> = {}): GrFilterBarProps => ({
  ...
  warehouseOptions: [
    { value: '9701', label: WAREHOUSE_LABEL },
    { value: '9702', label: OTHER_WAREHOUSE_LABEL },
  ],
  receiptTypeOptions: [],
  statusOptions: [],
...
describe('GrFilterBar — 되돌림', () => {
  /**
   * **되돌림을 참조가 아니라 값으로 판정한다**(`omf-mes#43`). 부모는 렌더할 때마다 주소에서
   * 값을 새로 읽으므로 내용이 같아도 참조가 달라질 수 있고(조회 응답이 도착해 다시 그려질 때가
   * 그렇다), 참조로 판정하면 그때마다 사용자가 치던 값이 사라진다.
   */
...
describe('GrFilterBar — 값 목록이 확정되지 않은 코드', () => {
   ...
   */
  it.each([[t.fields.receiptType], [t.fields.status]])(
    '%s 선택지가 비어 있고 왜 비었는지 밝힌다',
    (label) => {
      renderBar();

      expect(screen.getByLabelText(label)).toHaveTextContent(messages.pendingCode.placeholder);
 ...
 * **창고 좁힘의 두 방향**(부품 수준).
 *
 * 화면이 좁힌 뒤 넘기므로 이 부품이 보는 것은 「몇 개가 왔는가」와 「안내가 붙었는가」뿐이다 —
 * 그 둘이 실제로 갈리는지를 여기서 재고, 좁힘 자체의 판정은 `code-options.test.ts`가 잰다.
 */
describe('GrFilterBar — 폐기 대상 창고 좁힘', () => {
  it('좁히지 못한 동안에는 전체 창고와 그 사실을 함께 낸다', () => {
    renderBar({ warehouseNote: t.filters.warehouseTypePending });
  ...
  it('좁혀지면 안내가 사라진다', () => {
    renderBar({ warehouseOptions: [{ value: '9701', label: WAREHOUSE_LABEL }] });
...
describe('GrFilterBar — 조건 칩', () => {
  ...
  /** **번호를 문구로 만드는 자리가 없다**(`omf-mes#44`) — 화면이 이름으로 풀어 넘긴다. */
  it('칩에 내부 번호가 없다', () => {
    renderBar({ appliedFilters: applied });
  ...
  /**
   * **기간 칩에는 ×가 없다** — 날짜 컨트롤이 값을 개별로 비우는 수단을 주지 않아 눌러도 값이
   * 남는다. **이름이 아니라 개수로 잰다** — 「입고일 조건 해제」라는 이름의 ×가 없는지만 보면
   * 다른 이름을 단 ×를 놓친다.
   */
   ...
   * 아니라 「칩이 하나도 없으면」을 재는 것이 되어, 판정 기준을 갈아 끼워도 지나간다.
   */
...
/**
 * **이 회차에는 잠글 것이 없다.** 쓰기가 하나도 없어 「전송 중」 상태 자체가 서지 않는다 —
 * 그 잠금은 쓰기가 생기는 회차에서 이 부품에 더해진다. 지금은 조회·초기화가 늘 열려 있다는
 * 사실을 고정해 두어, 잠금이 더해질 때 그 변화가 감지기에 잡히게 한다.
 */
describe('GrFilterBar — 조회와 초기화', () => {
  it('조회와 초기화가 늘 열려 있다', () => {
    renderBar();
