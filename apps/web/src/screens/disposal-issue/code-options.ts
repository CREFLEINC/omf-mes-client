 ...
 * | **폐기 요청 정보 — 폐기 거래처** | `DISPOSAL_PARTNER_ROLE_CODE` | **더는 막지 않는다** — 채워졌다(2026-08-16) |
 *
 ...
 *
 * **한 줄은 채워졌다**(2026-08-16 업무 확정 — 폐기 거래처 역할 코드). 그 줄을 표에서 지우지
 * 않는다 — **비면 무엇이 막히는가**를 적은 표라, 채워진 뒤에도 그 자리가 무엇을 여는 열쇠였는지
 * 남아 있어야 누군가 다시 비웠을 때 값이 나간다. 나머지는 그대로 비어 있다.
 *
 ...
 * 잠정**이며, 확정되면 **이 한 줄만** 바뀐다. `code-options.test.ts`가 이 글자를 **리터럴로
 * 고정**해, 다른 철자가 오면 그 시험이 울어 고칠 자리를 가리킨다.
 *
