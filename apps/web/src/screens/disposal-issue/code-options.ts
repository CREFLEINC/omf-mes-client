 ...
 * **목록을 좁히는 조건 한 줄**이다.
 *
 * ✅ **영문 표기는 확정됐다**(#173). 뜻(「폐기 처리를 담당하는 업체」)은 업무가 못 박았고,
 * 계약이 역할 코드를 다섯으로 **enum 편입**하면서 글자도 확정됐다 — 어휘 밖 값은 서버가
 * 400으로 거절한다. 그래서 이 줄의 타입이 계약에서 파생되고, 계약이 값 이름을 바꾸면
 * **여기서 컴파일이 멈춘다.** 어느 다섯인가는 `code-options.test.ts`가 이 글자를
 * **리터럴로 고정**해 다른 철자가 오면 운다.
 *
