...
import type { DisposalLineRow, DisposalSelectionView } from './disposal-selection';
import {
  describeReference,
  isLotHeld,
  toReference,
...
export interface GrLineColumnsInput {
  ...
  /**
   * 지금 아무 줄도 고칠 수 없는가. **뒤 회차의 전송 중 잠금이 이 자리로 들어온다** —
   * 이 회차에는 쓰기가 없어 늘 거짓이다.
   *
   * 고를 수 없는 줄의 잠금(`blocked`)과 **뜻이 다르다.** 그쪽은 「이 줄은 폐기할 수 없다」이고
   * 이쪽은 「지금은 아무 줄도 못 고친다」다 — 그래서 사유도 갈라 붙인다.
   */
  isLocked: boolean;
 ...
 *
 * **이 표가 스펙 5열의 나머지 셋을 실현한다**(승인 기록 정정 2). 대상 표(6열)는 전표 한 건을
 * 고르는 자리이고, 「무엇을(품목)·어느 LOT을·그 창고에 얼마나 있는가(보유 수량)」는 라인·잔액
 * 수준의 값이라 전표 목록 응답에 없다 — 계약에 입고 라인의 전역 목록 경로가 없어 한 표로는
 * 만들 수 없고, 두 표가 2단으로 그 다섯을 함께 낸다.
 *
 * **단위 열을 따로 두지 않고 수량 표기에 붙인다**(「100 SAMPLE-UOM-EA」 — W-01-03이 세운 처리).
 ...
 * | **폐기 수량** | 136px | `sm` `TextField` + 인라인 오류 + **잠금 사유** |
 * | **지정 폭 합** | **720px** | |
 * | 흡수 열 예산 | 184px | 「코드 · 이름」이 접히지 않고 읽히는 하한 |
 * | **합** | **904px** | `58rem`(928px) 안에 들어간다 — 24px 여유 |
 *
 * **흡수 열이 실제로 받는 폭은 208px**(928 − 720)로 예산 184px보다 넓다. 지정 폭 합이 표
 * 하한에 가까우면 흡수 열이 몇십 px밖에 못 받아 「코드 · 이름」이 낱말 단위로 쪼개진다 —
 * 그 어긋남은 열 폭 합만 세는 단언을 통과하므로 **남는 폭까지 함께 단언한다**(완료 조건 C31).
 *
 * **잠금 사유가 폐기 수량 칸에 붙는 이유**: 선택 열은 체크박스 하나 폭(56px)이라 문장이 들어갈
 * 자리가 없다. 사유는 넓은 칸에 한 번만 그리고, **선택칸과 수량칸이 함께**
 * `aria-describedby`로 그것을 가리킨다 — 같은 문장을 두 번 그리면 화면이 같은 말을 되풀이한다.
 *
 * 디자인 시스템 `TextField`에 `disabledReason`이 있고 그것도 사유를 보이는 글자로 내고
 * `aria-describedby`까지 잇는다(설치본 실측). **그런데도 쓰지 않는다** — 그 prop은 사유를
 * `TextField` 안에 가두어 만들어지는 `id`를 밖에서 알 수 없고, 그러면 **선택칸이 같은 사유를
 * 가리킬 수 없다.** 우리가 `id`를 쥐는 형태라야 두 컨트롤이 한 문장을 공유한다.
 *
 * **판정을 여기서 만들지 않는다**(감지기 M24·M31). 고를 수 있는가·골라졌는가·오류가 무엇인가는
 * 전부 `disposal-selection.ts`가 낸 줄에 실려 온다. 표가 다시 판정하면 표시와 다음 단계의
 * 판정이 갈린다.
 */
export const buildGrLineColumns = ({
  ...
  onToggleSelect,
  onChangeQty,
}: GrLineColumnsInput): Column<DisposalLineRow>[] => {
  const reasonIdOf = (row: DisposalLineRow): string => `${reasonIdPrefix}-${String(row.ordinal)}`;
  const uomNameOf = (row: DisposalLineRow): string =>
  ...
  return [
    ...
    },
    {
      ...
      width: '168px',
      /*
       * **보류 표식은 이름 옆에 글자로 붙는다**(색에만 기대지 않는다). 폐기해도 보류가
       * 유지된다는 사실을 사용자가 이 자리에서 읽는다 — **막지는 않는다.** 보류·차단된
       * 자재를 장부에서 덜어 내는 것이 이 화면의 주 용도이며, 그래서 `line-select.ts`도
       * 보류를 보지 않는다.
       */
      render: (row) => (
        <>
    ...
    },
    {
      ...
      width: '120px',
      /*
       * **모르는 것을 아는 척하지 않는다**(계획 결정 4). 못 구한 자리에 `0`이나 「—」를 내면
    ...
    {
      key: 'disposalQty',
      header: t.lineTable.disposalQty,
      width: '136px',
      /*
       * **표 안의 입력칸.** 보이는 라벨을 둘 자리가 없어 `aria-label`로 준다(배치 규범 3의
       * 이탈 조건). 이름에 **표시 순번**을 넣는다 — 「폐기 수량」이 줄마다 되풀이되면 어느 줄인지
        ...
        return (
          <div className="field-cell">
            <TextField
              ...
              fullWidth
              inputMode="decimal"
              aria-label={t.lineTable.disposalQtyLabel(row.ordinal)}
              aria-describedby={blocked ? reasonIdOf(row) : undefined}
              value={row.qtyText}
              disabled={blocked || isLocked}
              error={row.error}
...
export interface GrLineTableProps extends Omit<GrLineColumnsInput, 'reasonIdPrefix'> {
  ...
  /**
   * 고른 줄의 요약과 판정. **화면이 한 번 부른 결과를 받는다**(감지기 M31).
   *
   * 이 표가 직접 부르면 요약과 뒤 회차의 버튼 판정이 둘로 갈린다 — 같은 함수라도 부르는
   * 자리가 둘이면 한쪽 인자만 바뀌었을 때 표와 버튼이 서로 다른 말을 한다.
   */
  selection: DisposalSelectionView;
  onRetryReferences: () => void;
  onRetryBalances: () => void;
}

/**
 * 고른 입고 전표의 라인 표 — **무엇을 얼마나 폐기할지 정하는 자리다.**
 *
...
export const GrLineTable = ({
  ...
  return (
    <>
      <p className="field-note">{t.notes.disposalQtyEmptyStart}</p>
      ...
      {/* 무엇이 나갈지 표 바로 아래에서 읽힌다 — 뒤 회차의 확인 창에서 처음 보게 하지 않는다. */}
      <p className="field-note">{summary()}</p>

      {selection.ready.kind === 'blocked' && <p className="field-note">{selection.ready.reason}</p>}
