 ...
 * (완료 조건 C27·C28 · 감지기 M24·M28·M29·M31).
 *
 * 표가 「고를 수 있다」를 자기대로 판정하고 다음 단계가 「보낼 수 있다」를 또 자기대로 판정하면
 * 둘이 갈린다 — 표에는 멀쩡한데 버튼이 잠겨 있거나, 반대로 잠겼어야 할 것이 열린다.
 * 이 화면이 만드는 것은 **승인을 받아 재고를 덜어 내는 전표**라 뒤쪽 어긋남이 비싸다.
 *
 * 그래서 부품은 **판정하지 않고 받는다.** 표는 `DisposalLineRow`를 그대로 그리고, 사유·요약도
 * 여기서 나온 문장을 낸다.
 *
...
export interface DisposalLineRow {
  line: ReceiptLineView;
  /**
   * 표시 순번(1부터). 표 안 컨트롤의 접근 이름과 사유 `id`가 쓴다.
   *
   * **내부 번호를 쓰지 않는다**(`omf-mes#44`) — `id` 속성은 화면 글자가 아니지만 DOM에 남고,
   * 한 번 쓰기 시작하면 접근 이름으로도 새기 쉽다. 이 표에는 줄번호 열이 없고(계획 §5.5)
   * 품목·LOT은 아직 이름이 안 풀렸거나 줄끼리 겹칠 수 있어 줄을 가르지 못한다.
   */
  ordinal: number;
 ...
 * 되는 이유다(수명 표 14행).
 */
export const toDisposalLineRows = (
  ...
  lines.map((line, index) => {
    ...
    const qty = parseDisposalQty(qtyText);
 ...
 */
export type DisposalReadyState = { kind: 'ready' } | { kind: 'blocked'; reason: string };

export interface DisposalSelectionView {
  /** 실제로 보낼 줄. **요약도 뒤따르는 회차의 요청도 이 배열에서 나온다.** */
  selectedRows: DisposalLineRow[];
  count: number;
  /**
   * 고른 줄의 폐기 수량 합. **단위가 섞이면 `null`이다** — 100 개와 5 상자를 더한 105는
   * 어떤 뜻도 없고, 화면이 확인하지 않은 것을 말하는 것이 된다.
   */
  totalQty: number | null;
  totalUomId: number | null;
  ready: DisposalReadyState;
}

/**
 * 고른 줄을 세고 다음 단계로 갈 수 있는지 판정한다.
 *
 * 사유 순서가 뜻을 정한다 — **아무것도 안 골랐다 → 빈 칸 → 오류.** 빈 칸이 오류보다 앞서는
 * 이유는 오류가 이미 그 칸 아래에 붉은 글씨로 서 있는 반면, **빈 칸은 멀쩡해 보이기** 때문이다.
 */
export const describeDisposalSelection = (
  rows: readonly DisposalLineRow[],
): DisposalSelectionView => {
...
const sumQty = (rows: readonly DisposalLineRow[]): number =>
  rows.reduce((sum, row) => (row.qty.kind === 'qty' ? sum + row.qty.value : sum), 0);

const judge = (selectedRows: readonly DisposalLineRow[]): DisposalReadyState => {
