...
describe('buildGrColumns — 열 폭 예산', () => {
  ...
  it('지정 폭 합과 흡수 예산이 표 하한 안에 든다', () => {
    ...
    expect(TABLE_MIN_WIDTH_PX - fixed).toBe(248);
...
describe('GrTable — 행 표기', () => {
  ...
  it('참조가 아직 오지 않은 것과 목록에 없는 것을 가른다', () => {
    ...
    expect(screen.getAllByText(t.values.referenceLoading).length).toBe(goodsReceiptFixtures.length);
    expect(screen.queryByText(t.values.unknown)).not.toBeInTheDocument();
  });

  /**
   * **상태·유형 코드를 그대로 낸다.** 값 목록이 확정되지 않아 번역도 색 구분도 지어내는
   * 것이 된다(공유계약 G-2).
   */
  it('상태·유형 코드를 그대로 낸다', () => {
    renderTable();

    expect(screen.getAllByText('SAMPLE_GR_TYPE_A')).toHaveLength(2);
    expect(screen.getByText('SAMPLE_GR_STATUS_B')).toBeInTheDocument();
  });
});
