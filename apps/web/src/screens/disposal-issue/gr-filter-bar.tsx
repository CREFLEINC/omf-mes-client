...
export interface GrFilterBarProps {
   ...
   *
   * **좁힘도 화면이 한다.** 폐기 대상 창고 유형이 확정되면 그 유형만 넘어오고, 그전까지는
   * 전체가 넘어온다 — 부품이 자리표시 상수를 직접 읽으면 「값이 채워지면 좁혀진다」는 전환을
   * 화면 수준에서 잴 수 없다.
   */
  warehouseOptions: SelectOption[];
  /**
   * 창고 선택칸의 한계 안내(불러오기 실패 · 잘림 · **폐기 대상 창고를 가려내지 못함**).
   * 밝히지 않으면 사용자가 목록이 좁혀진 것으로 읽는다.
   */
 ...
 * 안내가 밝힌다.
 *
 * **창고를 화면이 골라 주지 않는다.** 폐기 대상은 불량 판정을 받아 들어온 자재이고 그것이
 * 놓이는 창고가 있으나, 창고 유형의 값 목록이 확정되지 않아 화면이 그 창고를 가려낼 수 없다 —
 * 그 사실은 선택칸 아래 안내가 말하고, 고르는 것은 사용자가 한다.
 *
