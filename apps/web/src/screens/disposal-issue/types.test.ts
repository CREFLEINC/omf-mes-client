...
describe('formatDateTime', () => {
  it('날짜와 분까지 공장 시각(UTC+7)으로 낸다', () => {
    expect(formatDateTime('2026-08-06T09:12:00+09:00')).toBe('2026-08-06 07:12');
  });

  /**
   * **공장 시각(베트남, UTC+7)으로 옮긴다(omf-all-around#20).** offset이 달라도 같은 순간이면
   * 같은 시각으로 보이고, 보는 사람의 실행 환경 시간대와는 상관없다 — 같은 전표가 사람마다
   * 다른 시각으로 보이지 않는다.
   */
  it('같은 순간이면 offset이 달라도 같은 공장 시각을 낸다', () => {
    expect(formatDateTime('2026-08-06T02:12:00Z')).toBe('2026-08-06 09:12');
    expect(formatDateTime('2026-08-06T11:12:00+09:00')).toBe('2026-08-06 09:12');
    expect(formatDateTime('2026-08-05T21:12:00-05:00')).toBe('2026-08-06 09:12');
  });

  it('다른 순간이면 다른 시각을 내고, 자정을 넘으면 날짜도 바뀐다', () => {
    expect(formatDateTime('2026-08-06T09:12:00Z')).toBe('2026-08-06 16:12');
    expect(formatDateTime('2026-08-06T09:12:00-05:00')).toBe('2026-08-06 21:12');
    expect(formatDateTime('2026-08-06T18:30:00Z')).toBe('2026-08-07 01:30');
  });
