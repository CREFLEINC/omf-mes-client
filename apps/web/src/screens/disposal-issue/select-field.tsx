 ...
 *
 * **잠금과 오류를 받지 않는다.** 이 회차에서 이 부품이 붙는 자리는 **조회 조건뿐**이고,
 * 조건 칸은 잠그지 않으며(배치 규범 3 — 잠그면 지금 걸린 조건을 읽을 수만 있고 고칠 수 없다)
 * 조건 값에는 서버가 오류를 돌려줄 자리가 없다. 폐기 정보 구획이 생기는 회차에서 그 둘이
 * 이 부품에 더해진다 — 쓰이지 않는 prop을 미리 두면 잠기지도 않는 칸에 잠금 규칙이 붙는다.
 *
