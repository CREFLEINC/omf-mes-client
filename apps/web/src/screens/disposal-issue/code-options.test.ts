 ...
 *
 * ⛔ **리터럴로 고정한다.** 영문 표기는 계약 enum 편입 전까지 잠정이라(사용자 결정 2026-08-16)
 * 상수를 그대로 견주면 어떤 글자로 바뀌어도 시험이 함께 따라가 아무도 울지 않는다. 계약이
 * 다른 철자를 확정하면 **이 줄이 울어야** 고칠 자리를 찾는다.
 * 짝이 되는 사본은 `screens/common-code/partner-role-vocab.ts`의 `PARTNER_ROLE_CODES.disposal`이며
 * **그쪽도 각자 리터럴로 고정한다** — 한쪽만 고치면 아무도 울지 않는다는 한계는 감추지 않는다.
 *
