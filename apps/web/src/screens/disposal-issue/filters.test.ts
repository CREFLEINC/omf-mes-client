...
const NAMES = { warehouse: 'SAMPLE-WH-01 · 합성 폐기창고 가' };
...
describe('readFilters', () => {
  ...
  /**
   * **기본 조건을 심지 않는다** — 창고도 기간도 비어 있는 것이 첫 진입의 상태다.
   * 창고를 심으면 화면이 「이 창고가 폐기 대상 창고다」를 지어내는 것이 된다(계획 결정 2).
   */
  it('기본값에 창고·기간이 없다', () => {
  ...
  /**
   * **위쪽 모서리를 함께 고정한다.** 상한을 16자리로 늘리면 `Number('9999999999999999')`가
   * `10000000000000000`으로 **조용히 바뀐다** — 안전 정수 범위를 넘기기 때문이다.
   * 아래 짝 방향 단언만으로는 그 한 칸이 재어지지 않는다.
   */
  ...
  /** 검색어만 원문 그대로 읽는다 — 친 공백이 말없이 사라지면 무엇을 쳤는지 되짚을 수 없다. */
  it('검색어는 원문 그대로 읽는다', () => {
    expect(readFilters(params('q=%20GR%20')).q).toBe(' GR ');
  });
});
...
describe('toFilterQuery', () => {
  ...
  /** **창고를 고르기 전에는 창고 조건이 실리지 않는다** — 빈 값을 `warehouseId=`로 싣지 않는다. */
  it('창고를 고르기 전에는 창고 조건이 없다', () => {
    expect(toFilterQuery(filters({ q: 'GR' }))).toEqual({ q: 'GR' });
  });
...
describe('toFilterChips', () => {
  ...
  /**
   * **기간 칩에는 해제 버튼이 없다.** 날짜 컨트롤이 값을 개별로 비우는 수단을 주지 않아
   * 기간을 푸는 길이 「초기화」뿐이다 — ×를 달면 눌러도 값이 남는다.
   */
  it('기간 칩만 해제 버튼이 없다', () => {
    const chips = toFilterChips(
  ...
  it('요청에 실리지 않는 값에는 칩을 내지 않는다', () => {
    expect(toFilterChips(filters({ warehouse: 'abc', from: '2026-13-01', q: '  ' }), NAMES)).toEqual(
      [],
    );
  });
});
