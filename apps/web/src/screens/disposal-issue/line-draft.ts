 ...
 * 응답이 들어가 **치던 값이 갱신에 사라진다**(`omf-mes#43` · 감지기 M30). 성기게 두면 되돌림
 * 축이 **고른 전표(`gr`) 하나**로 남고, 「빈 칸으로 시작한다」(완료 조건 C26 · 감지기 M26)가
 * 읽는 자리의 기본값으로 **구조에 박힌다** — 채우려면 이 파일의 두 함수를 고쳐야 한다.
 *
 * 사라진 줄의 초안이 남는 문제는 **표에 있는 줄만 세는 것**으로 막는다(`disposal-selection.ts`).
 *
 ...
 * 친 글자를 폐기 수량으로 읽는다.
 *
 * **빈 칸은 오류가 아니다** — 「이 줄은 폐기하지 않는다」는 뜻이다. 고른 줄인데 비어 있는
 * 사정은 다음 단계의 사유가 맡는다(`disposal-selection.ts`).
 *
 ...
 */
export const parseDisposalQty = (raw: string): QtyParse => {
  const text = raw.trim();
...
/**
 * 그 줄의 폐기 수량 칸에 보이는 글자. **초안에 없으면 빈 칸이다.**
 *
 * **이 자리가 「빈 칸으로 시작한다」를 지키는 곳이다**(완료 조건 C26 · 감지기 M26). 입고
 * 수량으로 되돌리려면 이 함수를 고쳐야 하고, 그러면 전량 폐기가 기본값처럼 보여 사용자가
 * 그대로 확인하는 순간 **받은 전부가 장부에서 빠진다.**
 */
export const readDraftQty = (draft: LineDraft, goodsReceiptLineId: number): string =>
  draft.qtyTexts[goodsReceiptLineId] ?? '';

/** 한 줄의 폐기 수량을 바꾼다. 앞 초안을 고치지 않는다 — 같은 참조를 고치면 화면이 다시 그려지지 않는다. */
...
export const hasAnyLineDraftValue = (draft: LineDraft): boolean =>
  draft.selectedLineIds.length > 0 || Object.values(draft.qtyTexts).some((text) => text !== '');

/** 그 줄이 초안에서 골라져 있는가. **고를 수 있는 줄인지는 여기서 보지 않는다**(`disposal-selection.ts`의 몫). */
 ...
 * 처음부터 다시 친다 — 무엇을 보내는지는 요약(고른 줄 수·합계)이 밝히므로, 남은 값이 조용히
 * 나가는 일은 없다.
 */
