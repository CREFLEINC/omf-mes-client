...
import type { LookupEntry, PageMeta, WarehouseEntry } from './types';

/**
 * 내부 번호(FK)로 이어진 값을 이름으로 푸는 참조. **이 회차에는 창고 하나다.**
 *
 ...
 * 미사용 값을 참조하는 과거 입고가 오면 이름이 비어 보인다.
 *
 ...
 * | 참조 | 보이는 자리 | 복구 | 언제 부르나 |
 * | --- | --- | --- | --- |
 * | 창고 | 조건 줄 · 목록 표 | **목록 구획** | 첫 진입 |
 *
 * 품목·단위·자재 LOT·위치는 **라인 표가 그려지는 회차**에서 이 파일에 더한다 — 그 표 자체가
 * 상세 응답을 기다리므로 미리 받아 둘 이득이 없고, 지금 부르면 첫 진입의 요청 수만 이유 없이 는다.
 *
...
/** 창고 참조. **유형 코드를 함께 나른다** — 그것이 다른 참조와 다른 유일한 점이다. */
...
const EMPTY_ENTRIES: WarehouseEntry[] = [];
 ...
 *
 * **첫 진입에 부른다** — 조건 줄과 목록 표가 첫 화면부터 이 이름을 쓴다.
 */
