...
export const OverReceiptSplitScreen = () => {
  ...
  const submit = (purchaseOrder: PoView, mode: SplitMode): void => {
     ...
     *
     * **매 제출마다 새로 적는다.** 한 번이라도 참이면 참으로 두면 앞의 초과분 등록이 뒤의
     * 정량분 전표에 진입로를 되살린다 — 이 화면은 성공한 뒤에도 그 자리에 남는다.
     *
     * 이 값이 참일 때 **실을 초과분 줄이 실제로 있다**는 것은 `validation.ts`의 `canSubmit`이
     * 진다(초과분 0행이면 그 두 갈래의 버튼이 잠긴다). 그 잠금이 느슨해지면 초과분 없는 등록이
     * 「초과분이 실렸다」고 말하게 되고, 정량분 전표에 진입로가 서는 결함이 다른 문으로 돌아온다.
     */
