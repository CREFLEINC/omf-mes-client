 ...
 * 대상 「열기」의 판정.
 *
 * **결재함은 대상 문서의 내용을 그리지 않는다** — 계약이 그렇게 적었고 상세 응답에 대상
 * 본문이 아예 없다. 이 화면이 대상에 대해 할 수 있는 일은 **이름을 내고 원 화면을 여는 것**
 * 둘뿐이며, 열 수 없으면 **왜 못 여는지**를 말한다.
 *
 * **`targetTypeCode`로 분기하지 않는다.** 유형을 보고 화면을 고르는 매핑표는 계약이 명시적으로
 * 금지했다(그 판정은 서버가 `screenId`·`openable`로 이미 내려 준다). 이 파일에도, 이 슬라이스
 * 어디에도 그 코드로 갈리는 줄이 없다.
 *
 ...
 * | `noScreenId` | 열 수 있다는데 **어느 화면인지 오지 않았다** | 계약·규약(§5.4-7의 질문) |
