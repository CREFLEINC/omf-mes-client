 ...
 * | 조건·탭·쪽이 바뀌면 고른 요청(`rq`)이 사라진다 | `toSearchParams`가 그 자리를 **만들지 않는다** |
 * | 기본값은 주소에 적지 않는다(기본 탭·첫 쪽) | `toSearchParams` |
 * | 상신일을 **`YYYY-MM-DD` 그대로** 보낸다 | `toRequestListQuery` |
 *
 * 이 화면이 소유한다 — 다른 화면 슬라이스의 같은 이름 파일을 참조하지 않는다.
 * 날짜 조건을 다루는 화면이 이미 있으나 그쪽은 계약이 `date-time`이라 RFC3339로 바꿔 보낸다.
 * **여기서 그 변환을 베끼면 틀린다** — 이 계약의 두 파라미터는 `format: date`다.
 */
...
export const readFilters = (params: URLSearchParams): InboxFilters => ({
...
/** 주소에서 탭 값만 꺼낸다. 뜻을 정하는 것은 `tabs.ts`다(권한 자리표시가 거기 있다). */
export const readTabParam = (params: URLSearchParams): string | null => params.get(URL_KEYS.tab);

/**
 * 조건·탭·쪽 전체를 주소로 옮긴다. **빈 조건과 기본값은 키 자체를 두지 않는다** —
 * 주소가 조건을 그대로 드러내야 무엇으로 조회했는지 읽을 수 있고,
 * 같은 화면의 주소가 두 가지가 되면 공유·뒤로가기가 갈린다.
 *
 * **고른 요청(`rq`)을 담지 않는다.** 조건·탭·쪽이 바뀌면 보이는 행이 달라지므로
...
export const toSearchParams = (
  ...
  if (tab !== DEFAULT_TAB) next.set(URL_KEYS.tab, tab);
 ...
 * 선택을 얹은 주소.
 *
 * **조건·탭·쪽은 그대로 두고 선택 자리만 다시 세운다** — 요청을 고르는 것은 보이는 행을
...
export const toSelectionSearchParams = (
  ...
  const next = toSearchParams(filters, tab, page);
...
/**
 * 서버로 보낼 조회 쿼리. 탭의 축과 조건이 여기서 한 벌로 합쳐진다.
 *
...
/**
 * 조건 칩의 키. **`InboxFilters`의 키와 같지 않다** — 상신일 구간은 두 필드지만
 * 사용자에게는 조건 하나라, 칩도 하나이고 풀 때도 두 끝이 함께 풀려야 한다.
 */
export type FilterChipKey = 'approvalTypeCode' | 'statusCode' | 'period' | 'q';
...
export const toFilterChips = (filters: InboxFilters): InboxFilterChip[] => {
 ...
 */
export const clearFilter = (filters: InboxFilters, key: FilterChipKey): InboxFilters =>
