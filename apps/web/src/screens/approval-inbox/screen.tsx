 ...
 *
 * **이 회차에 결재가 붙고 화면이 열린다.** 고른 요청의 정보·대상·결재 진행 아래에 결재 구획이
 * 서고, 라우트와 사이드바에 이 화면이 등록된다 — 결재할 수 있게 된 다음에 노출한다.
 *
 * ---
 *
 ...
 *
 * **결재는 S1에서만 성립한다.** 승인·반려에 필요한 잠금 토큰이 상세 200으로만 오므로,
 * S0·S2·S3에는 보낼 수단 자체가 없다 — 그 자리에 버튼을 두지 않는다.
 *
...
export const ApprovalInboxScreen = () => {
   ...
   * | 16 | **상세 조회 실패**(403·500·재조회 실패) | 유지 | 유지 | 유지 | 유지 | 유지 | **닫는다** | 유지 |
   *
   * **16행은 리뷰가 찾아 준 사각이다.** 조회 라이브러리는 **재조회가 실패해도 마지막 성공
   * 자료를 남긴다** — 그래서 「자료가 있는가」만 묻는 조건은 **「지금 읽을 수 있는가」를 묻지
   * 않는다.** 그 틈에서 아래 구획은 「볼 권한이 없습니다」로 갈리는데 확인 창은 그대로 서고,
   * 그 창의 확인이 **되돌릴 수 없는 결재를 실제로 보냈다.**
   *
   * `rq`를 비우지 않는 이유는 7행과 같다 — 읽지 못하는 것과 없는 것은 다르다. 초안도 남긴다:
   * 상세가 돌아오면 그대로 이어서 결재한다. **닫는 것은 창뿐이다.**
   *
   * **11~14행이 계획의 초안과 다르다.** 계획을 쓸 때는 의견 입력칸이 **창 안**에 있었고,
   * 그때는 「창을 닫으면 초안과 배너를 비운다」·「실패하면 창을 연 채로 둔다」가 맞았다.
   * 입력칸이 **구획으로 내려오면서**(설계 스펙 대조 결과) 그 둘이 모두 뒤집힌다:
   *
   * - **12행 — 창을 닫아도 초안을 비우지 않는다.** 이제 창을 닫는 것은 「고치러 나가는 길」이다.
   *   비우면 사용자가 방금 적은 반려 사유를 **취소를 누른 대가로** 잃는다.
   * - **14행 — 실패하면 창을 닫는다.** 창은 입력을 받지 않으므로 열어 둔 채로는 **고칠 수가
   *   없다.** 오류를 보여 주고도 손댈 수 없는 창은 막다른 길이다. 배너는 입력칸 바로 위,
   *   곧 고칠 자리 옆에 선다.
   * - **11행 — 창을 열어도 배너를 비우지 않는다.** 지난 시도가 실패한 것은 아직 참이고,
   *   보낼 때 공통 쓰기 훅이 스스로 지운다. 열자마자 지우면 창을 닫은 뒤 **왜 다시 보내려
   *   했는지**가 사라진다.
   *
   * 표를 통째로 적는 이유는 **표에 오르지 않은 상태가 규칙이 닿지 않는 사각**이 되기
   * 때문이며, 열여섯째 조작이 생기면 규칙을 정하기 전에 이 표에 행부터 더한다.
   *
   * **왜 이렇게 정했는가**
   *
   ...
   * - **13행이 `rq`를 유지하는 이유**: 승인하면 그 요청은 「내 결재 대기」에서 빠진다.
   *   그때 선택까지 비우면 사용자가 **방금 자기가 무엇을 했는지 확인할 자리**를 잃는다.
   *   목록에 없는 선택을 바깥에서 정리하지 않는 것은 6·7행과 같은 규율이다 —
   *   **읽는 자리(상세 조회)가 판정한다.**
   * - **15행이 `rq`까지 잠그는 이유**: 나가는 중인 결재의 결과가 **다른 요청의 맥락에**
   *   나타나면 사용자는 그것이 무엇에 대한 답인지 알 수 없다.
   *
   ...
   * 4. 초안·창·배너를 거두는 자리는 **`decisionTargetKey`에 묶인 effect 한 곳**이다.
   *    같은 이유다 — 클릭에만 두면 주소로 들어온 길에서 앞 요청의 의견이 살아남는다.
   * 5. **사용자가 대상을 바꾸는 길은 전부 한 문(`applyUserNavigation`)을 지난다.**
   *    15행의 잠금이 그 문 하나에만 있어야 탭·목록 행·조건 칩·쪽이 갈리지 않는다.
   */
  /*
   ...
   * | --- | --- | :-: | --- | --- |
   * | 목록 `GET /app/approval-requests` | — | **주지 않는다** | — | — |
   * | 건수 `GET …?myTurnOnly&size=1` | — | 주지 않는다 | — | — |
   * | **상세 `GET /{id}`** | — | **준다** | **상세 경로** ← 토큰을 확보하는 **유일한 자리** | — |
   ...
   *
   * **의존성은 하나뿐이다.** 초안·응답 배열·`reset` 함수 참조를 넣으면 의견을 한 글자 칠
   * 때마다 배너가 지워지거나(너무 지움) 아예 보이지 않는다(늘 지움).
   */
  useEffect(() => {
   ...
   *
   * 첫째 겹은 컨트롤의 `disabled`인데 **탭·목록 행·조건 칩·쪽 이동은 잠금을 받지 않는다** —
  ...
  const apply = (nextFilters: InboxFilters, nextTab: InboxTab, nextPage = 1): void => {
    applyUserNavigation(toSearchParams(nextFilters, nextTab, nextPage));
  };
  ...
  /**
   * 결재를 **실제로 보내는 유일한 자리** — 여기서 한 번 더 본다.
   *
   ...
   * | 2 | **상세를 읽을 수 있는가** | 아니다(**둘째 겹**) | 창의 마운트 조건(`decisionDialog`) — 「상세가 500·403이면 창이 사라지고 쓰기가 나가지 않는다」가 잰다 |
   * | 3 | **내 차례인가** | **그렇다** | 창이 열린 사이 상세가 새로 와 차례가 끝나면 창은 남아 있다 — 여기가 유일한 문이다(M34) |
   * | 4 | **반려 본문이 서는가** | **그렇다** | 창이 열린 사이 의견이 비는 길이 있다. **목이 공백만인 의견을 200으로 받아** 막는 곳이 화면뿐이다(M35) |
   *
   * 1·2를 남기는 이유: 첫째 겹은 **렌더 층**에 있어 한 프레임 늦게 반영되는 반면 이 함수는
   * 그 사이 도착한 클릭도 받는다. 되돌릴 수 없는 조작에서 그 폭만큼은 값이 있다.
   */
