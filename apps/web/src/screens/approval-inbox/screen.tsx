...
export const ApprovalInboxScreen = () => {
   ...
   * 재판정은 셋이다.
   *
   * 1. **연타·중복** — 이미 나가는 중이면 보내지 않는다. 공통 훅이 호출마다 새 멱등 키를
   *    만들어(`omf-mes#55`) 두 번 나가면 서버에는 **다른 요청 둘**로 보인다.
   * 2. **내 차례인가** — `isMyTurn`을 **다시 읽는다**. 단계 배열로 계산하지 않는다(결정 7).
   * 3. **반려 본문이 서는가** — 다듬은 의견이 비면 보내지 않는다. **목이 공백만인 의견을
   *    200으로 받으므로 막는 곳이 화면뿐이다.**
   */
  const confirmDecision = (kind: DecisionKind): void => {
    ...
    if (data === undefined || selectedRequestId === null || !data.request.isMyTurn) {
      setDialogKind(null);
  ...
  const decisionDialog = (): ReactNode => {
    if (dialogKind === null || detail.data === undefined) return null;
