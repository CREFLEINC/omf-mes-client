 ...
 *
 * ⚠ **확정(`:pack`)의 응답 목록에 409 가 없다**(200·400·403·404). 그런데 그 오퍼레이션의
 * «설명»은 「이미 확정된 포장은 409 다」로 적는다 — **계약 안에서 갈린다.** 화면은 선언된
 * 갈래를 다루고 409 는 일반 거부로 받는다(설계팀 확인 대상).
 *
...
const toContent = (error: ApiError): BannerContent => {
  switch (error.kind) {
    ...
    /*
     * ⚠ **응답 목록에 없는 갈래다** — `:pack` 은 200·400·403·404 만 선언한다. 그래도 갈래를
     * 비워 두지 않는다: 오퍼레이션 «설명»이 409 를 말하고 있고, 서버가 그대로 낼 수 있다.
     *
