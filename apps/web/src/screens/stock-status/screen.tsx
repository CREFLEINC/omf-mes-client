...
export const StockStatusScreen = () => {
  ...
  /*
   * 고른 거래. **이력 조회가 성립할 때만 뜻이 있다** — 기간이 없으면 위 목록이 비어 있어
   * 라인 표만 홀로 열린 상태가 된다.
   *
   * **잣대는 「성립하는가」이지 「성공했는가」가 아니다.** 실패는 `historyQuery`를 비우지
   * 않으므로 이력이 실패한 동안에도 고른 거래의 라인은 조회된다. 일부러 그렇게 둔다 —
   *
   * - `tx`는 **사용자가 실제로 고른 거래**다. 주소에만 남은 지어낸 값이 아니다.
   * - 이력이 회복되면 그 사이 받아 둔 상세가 **캐시로 쓰여** 왕복이 준다. 멎게 하면 회복할
   *   때마다 왕복을 다시 친다.
   * - 잘못된 자료가 보이지 않는다 — 기간·쪽이 바뀌면 `tx`가 비므로(수명 표 9·11행)
   *   보이지 않는 거래를 가리킨 채 열리는 상태가 생기지 않는다.
   * - **같은 화면의 위 구획이 이미 그 형태다** — 잔액 목록이 실패해도 LOT 상세는 조회된다.
   *
   * 위 문단의 「부르고 나서 구획도 없이 버린다」는 **성립하지 않는 조회**를 가리킨다 —
   * 기간이나 LOT이 없으면 그 요청은 영영 쓰이지 않는다. 실패한 조회는 다시 성공할 수 있어
   * 성격이 다르다. 두 갈래가 달리 동작하는 이유가 이것이고, 이 문단이 그 근거다.
   */
