 ...
 *
 * ⭐ **서버는 발행 기록을 만들고 그림을 그려 줄 뿐이고, 물리 인쇄는 셸이 한다**(공유계약 K-4 —
 * 인쇄는 트랜잭션 밖). 그래서 이 파일은 **서버를 부르지 않는다** — 셸이 있는지 보고, 있으면
 * 넘기고, 결과를 사실대로 돌려준다.
 *
 * ⛔ **셸이 없을 때 성공으로 접지 않는다.** 이 화면을 브라우저에서 열면 통로가 없다. 그때 인쇄를
 * 「성공」으로 보고하면 **나오지 않은 라벨이 나온 것으로 남는다** — 계약이 인쇄 보고 경로를 따로
 * 둔 이유가 그것이다(공유계약 F-6).
 *
 * ⚠ **슬라이스가 소유한다.** 같은 모양이 `screens/goods-issue-qr/pop-print.ts` 에도 있는데, 화면
 * 슬라이스끼리는 서로 참조하지 않는 것이 이 저장소의 의존 규칙이다(`.dependency-cruiser.cjs`).
 */
 ...
 *
 * ⚠ **모듈 최상위 상수로 굳히지 않는다** — `window.pop` 은 preload 가 세우고, 부르는 시점이
 *   뜻을 갖는다.
 */
