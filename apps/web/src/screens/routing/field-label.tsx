 ...
 *
 * 디자인 시스템 `TextField`·`Select`의 내장 라벨은 `label` prop이 문자열이라
 * 필수 표시를 끼워 넣을 자리가 없다. 문자열에 `*`를 붙이면 **접근성 이름이 「이름 *」이 되어**
 * 라벨 조회가 깨지므로, 라벨을 직접 만들고 표시는 `<label>` 밖에 둔다.
 ...
 *
 * 값을 보여 주기만 하는 자리(판 번호·상태)는 이 부품을 쓰지 않는다 —
