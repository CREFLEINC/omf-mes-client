 ...
 *
 * 계약이 `createdFrom`·`createdTo`를 필수로 두고 `date-time`을 요구한다. 날짜만 보내거나
 ...
 *
 * **종료는 그날 23:59:59다.** 00:00:00으로 만들면 마지막 날에 생긴 건이 전부 빠져
 * 사용자에게는 「오늘 실패한 건이 안 보인다」로 나타난다.
 *
...
export const toPeriodQuery = (input: PeriodInput, offsetMinutes: number): PeriodQuery => {
  ...
  return {
    createdFrom: `${input.from}T00:00:00${zone}`,
    createdTo: `${input.to}T23:59:59${zone}`,
  };
};
