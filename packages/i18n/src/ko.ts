...
const overReceiptSplit = {
  ...
  actionReasons: {
    ...
    /* 이동할 화면이 아직 없다. 사유를 감추고 버튼만 두면 눌러도 아무 일이 없는 버튼이 된다. */
    createPurchaseOrderUnavailable:
      '신규 P/O 등록은 아직 열 수 없습니다. 발주 등록 화면이 준비되면 이 버튼을 쓸 수 있습니다.',
  },
  loading: {
...
const stocktaking = {
  ...
  actions: {
    ...
    /**
     * **자리만 두고 이동시키지 않는다**(착수 이슈 §5 ⚠). 재고 조정 화면(W-01-12)이 아직
     * 나가지 않았고 승인 계약도 없다 — 링크를 만들면 닿을 수 없는 곳으로 가는 길이 생긴다.
     */
    adjustment: '조정 등록',
  },
  /**
  ...
  actionReasons: {
    ...
    /** 조정 화면이 아직 없다(착수 이슈 §5 ⚠ · `omf-mes#66`). 자리만 두고 이동시키지 않는다. */
    adjustmentPending:
      '조정 등록: 재고 조정 화면이 아직 나가지 않아 지금은 이어서 할 수 없습니다. 준비되면 이 자리에서 이어집니다.',
...
const disposalIssue = {
  ...
  actions: {
    ...
    /**
     * **버튼 하나가 요청 둘을 잇는다**(승인 기록 정정 1-1) — 전표를 만들고 곧바로 상신한다.
     * 「등록」과 「상신」으로 나누어 적지 않는 이유는 사용자의 목적이 하나이기 때문이고,
     * 둘 중 하나만 이루어진 중간 상태는 **결과 구획이 정확히 말한다.**
     */
    submitDisposal: '품의 상신',
    /**
     * 이력 탭의 미상신 전표를 이어서 상신한다. **「상신」이 아니라 「재상신」이다** —
     * 이 자리에 오는 전표는 전표 생성까지는 끝났고 상신만 남은 것이라, 사용자가 무엇을
     * 되풀이하는지 낱말이 밝힌다.
     */
    resubmit: '재상신',
    /**
     * 승인이 끝난 품의를 실제 출고로 처리한다 — **이 화면에서 재고를 움직이는 유일한 조작**이다.
     * 「처리」로만 적지 않는 이유는 무엇으로 처리하는지가 낱말에 있어야 하기 때문이다.
     */
    postIssue: '기타출고 처리',
    /** 처리 확인 창의 바닥 버튼. 「확인」으로 적지 않는다 — 무엇을 누르는지가 낱말에 있어야 한다. */
    confirmPost: '처리',
    /** 되돌릴 수 없는 조작의 창에서 물러나는 길. 상신 창의 「계속 작성」과 쓰임이 다르다. */
    keepReviewing: '다시 확인',
    discardDrafts: '입력 지우기',
  ...
  errors: {
    ...
    /**
     * 상신 사유가 비었다. **공백만도 여기 온다** — 목이 공백만인 사유를 202로 통과시키므로
     * (실측) 막는 곳이 화면뿐이다. 인라인으로 내는 이유는 고칠 자리가 그 칸이기 때문이다.
     */
    reasonRequired: '상신 사유를 적어 주세요. 공백만으로는 상신할 수 없습니다.',
  },
  filters: {
    all: '전체',
    /**
   ...
   *
   * **「상신 사유」와 「폐기 사유」가 다른 값이다.** 앞은 결재에 올리는 문장(계약의
  ...
  form: {
    ...
    /**
     * 폐기 계정 칸이 잠긴 이유(완료 조건 C50 · 착수 이슈 §4). **감추지 않고 잠근 채로 보인다** —
     * 감추면 사용자는 이 화면에 없는 개념으로 읽고, 값이 왔을 때 놓을 자리도 사라진다.
     */
    disposalAccountPending: '폐기 계정은 확정 후 열립니다. 값이 정해지기 전에는 고를 수 없습니다.',
    /**
    ...
    /**
     * **한 번 눌러 요청이 둘 나간다**는 사실을 버튼 앞에서 밝힌다. 밝히지 않으면 「상신했는데
     * 전표만 생겼다」는 중간 실패가 사용자에게는 까닭 없는 일이 된다.
     */
    chainNote:
      '「품의 상신」을 누르면 폐기 품의 전표를 만들고 이어서 결재에 올립니다. 이때는 재고가 움직이지 않습니다 — 승인이 끝난 뒤 「기타출고 처리」에서 움직입니다.',
  },
  /**
  ...
  actionReasons: {
    codeListPending:
      '폐기 품의에 필요한 코드 값이 아직 확정되지 않아 상신할 수 없습니다. 대상 확인까지는 지금도 할 수 있습니다.',
    needsCodes: '품의 정보의 코드를 모두 고르세요.',
    needsIssuedDate: '출고 일자를 고르세요.',
    needsIssuedTime: '출고 시각을 적으세요.',
    needsReason: '상신 사유를 적으세요.',
    nothingToDiscard: '지울 입력이 없습니다.',
    /** 이미 상신된 품의에는 재상신을 열지 않는다 — 되풀이하면 요청이 두 벌이 된다. */
    alreadySubmitted: '이미 상신된 품의입니다. 아래 결재 진행에서 어디까지 왔는지 볼 수 있습니다.',
    /**
     * 상신 여부를 확인할 수 없는 전표(`approval-progress.ts`의 셋째 갈래). **미상신으로 접지
     * 않는다** — 값이 실려 온 이상 이미 올라갔을 수 있고, 그때 재상신하면 요청이 두 벌이 된다.
     */
    submissionUnknown: '상신 여부를 확인할 수 없어 상신을 열지 않습니다. 담당자에게 확인해 주세요.',
    /**
     * 보내는 동안에는 만들어진 품의를 열지 않는다. 대상을 바꾸는 길이 전부 **한 문**을
    ...
    /**
     * 상신조차 되지 않은 전표는 승인이 있을 수 없다 — **화면이 값 유무로 확실히 아는 사실**이라
     * 여기서는 잠근다(완료 조건 C69). 「승인이 끝났는가」를 모르는 것과 성질이 다르다.
     */
    postNeedsSubmission:
      '아직 상신되지 않은 품의입니다. 위에서 상신하고 승인을 받아야 처리할 수 있습니다.',
    /**
   ...
   * 확인 창 셋.
   *
   * **되돌릴 수 없는 조작 앞의 마지막 층이다.** 상신은 되돌릴 수 없고(반려된 뒤 다시 올리는 것은
   * **새 요청**이다) 그 뒤의 기타출고 처리는 재고를 움직인다 — 무엇이 나가는지 여기서 한 번 더
   * 보인다. **창 안에 선택칸을 두지 않는다**(`omf-mes#45`) — 고칠 것이 있으면 닫고 폼에서 고친다.
   */
  dialog: {
    submitTitle: '폐기 품의를 올릴까요?',
    submitLead: '아래 내용으로 폐기 품의 전표를 만들고 이어서 결재에 올립니다.',
    resubmitTitle: '이 품의를 결재에 올릴까요?',
    resubmitLead: '이미 만들어진 폐기 품의 전표를 결재에 올립니다. 전표를 새로 만들지 않습니다.',
    ...
    /**
     * **상신은 되돌릴 수 없다.** 계약의 출고 취소는 다시 결재를 타며 이 화면에 없다 —
     * 반려된 뒤 다시 올리는 것도 **새 요청**이다(스펙 §6).
     */
    submitNoUndo:
      '상신은 되돌릴 수 없습니다. 반려되면 다시 올릴 수 있으나 그때는 새 요청이 됩니다.',
    ...
    postLead: '아래 폐기 품의를 실제 출고로 처리합니다.',
    ...
    /** 상신 사유 첫 줄. **결재 진행에서 읽은 값이다** — 못 읽었으면 이 자리가 서지 않는다. */
    postReasonFirstLine: '상신 사유 첫 줄',
  },
  /**
   * 결과 구획 — **화면이 확인한 것만 말한다**(계획 결정 15).
   *
   * 이 구획의 중심은 **중간 상태를 숨기지 않는 것**이다(승인 기록 정정 1-1). 요청이 둘이라
   * 「전표는 만들어졌고 상신이 실패했다」가 실재하는 상태이며, 그때 화면은 통째로 실패라고도
   * 통째로 성공이라고도 말하지 않는다 — 그 전표는 **이력 탭에서 이어서 상신**할 수 있다.
   */
  result: {
    label: '상신 결과',
    createdTitle: (goodsIssueNo: string): string => `폐기 품의 전표 ${goodsIssueNo}를 만들었습니다`,
    submittedTitle: (goodsIssueNo: string): string => `${goodsIssueNo}를 결재에 올렸습니다`,
    submittedDescription:
      '결재가 시작됐습니다. 승인이 끝나면 「기타출고 처리」로 재고를 차감합니다.',
    /** 상신 결과에 **승인 요청 번호를 내지 않는다** — 응답이 내부 식별자 하나뿐이다(`omf-mes#44`). */
    submittedNoRequestNo: '승인 요청번호는 「처리 이력」 탭의 결재 진행에서 볼 수 있습니다.',
    /**
     * **부분 실패**(감지기 M57). 전표는 서버에 남았고 상신만 실패했다 — 통째로 실패로 말하면
     * 사용자가 처음부터 다시 만들어 **전표가 두 벌** 남는다.
     */
    partialTitle: (goodsIssueNo: string): string =>
      `전표 ${goodsIssueNo}는 만들어졌고 상신이 실패했습니다`,
    partialDescription:
      '전표를 다시 만들 필요가 없습니다. 「이 품의 열기」로 「처리 이력」 탭에서 이어서 상신하세요.',
    ...
    statusCode: '상태',
    /**
     * 상신 결과의 상태 — **전표를 만들 때 서버가 준 값**이다(리뷰 t5 M2).
     *
     * 이 구획은 「이번 세션에 내가 만든 전표의 기록」이고, 그 뒤에 상신·전기가 일어나면
  ...
  /**
   * 이력 탭의 재상신 구획 — **미상신 전표를 이어서 상신하는 자리다.**
   *
   * 전표 생성에 성공하고 상신에 실패한 전표가 실재하므로(요청이 둘이다) 그 전표를 되살릴 길이
   * 화면에 있어야 한다. **상신 규칙은 발의 자리와 같은 파일에서 나온다**(`reason-draft.ts`) —
   * 자리가 둘이어도 규칙이 둘이 되지 않는다.
   */
  resubmit: {
    label: '상신',
    lead: '이 품의는 아직 결재에 올라가지 않았습니다. 사유를 적고 결재에 올리세요.',
    submittedLead: '이 품의는 이미 결재에 올라가 있습니다.',
  },
  /**
  ...
  post: {
    label: '기타출고 처리',
    lead: '승인이 끝난 폐기 품의를 실제 출고로 처리하는 자리입니다.',
  ...
  notes: {
     ...
     * 「다시 시도하세요」로 끝나는데, 확인 없이 다시 보내면 같은 품의가 전표 두 벌로 남는다 —
     ...
     */
    reloadFailed:
      '최신 상태를 불러오지 못했습니다. 잠시 뒤 다시 시도하거나 「처리 이력」 탭에서 이어서 상신하세요.',
    /**
     * **응답을 받지 못한 처리에만** 한 줄을 더한다(다섯째 겹 · 감지기 M73). 상신의 같은 자리와
  ...
  progress: {
     ...
     * 「상신 사유 … 상신 사유, 그룹」으로 두 번 읽는다(전례 `iqc-skip-approval`과 같은 처리).
     */
    reasonPane: '상신 사유 전문',
    /**
