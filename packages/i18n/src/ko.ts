...
const supplierReturn = {
  ...
  empty: {
    ...
    /** 목록 조회가 실패해 고른 전표의 자리조차 판정할 수 없다. 복구 수단은 위 배너에 있다. */
    listFailedTitle: '목록을 불러오지 못해 고른 전표를 열 수 없습니다',
    listFailedDescription: '위의 「다시 시도」로 목록을 먼저 되살려 주세요.',
  },
  ...
  notes: {
    /**
     * 이 회차의 화면이 무엇까지 하는지 밝힌다 — 「반품 처리」 버튼이 없는 것이 고장으로
     * 읽히지 않게 한다.
     */
    readOnly: '이 화면은 지금 대상 입고 전표와 그 라인을 보는 데까지 열려 있습니다.',
    /**
  ...
  values: {
    ...
    /** 값이 **없는** 자리. 비워 두면 자료가 없는 것인지 화면이 빠뜨린 것인지 구분되지 않는다. */
    empty: '—',
    inactiveSuffix: ' (미사용)',
