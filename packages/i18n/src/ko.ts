...
const stockAdjust = {
  ...
  actionReasons: {
    ...
    /**
     * ⭐ **「값 목록이 확정되면 등록할 수 있습니다」가 여기서 걷혔다**(#36 회신 ④).
     *
     * 조정 사유는 고객이 공통코드 마스터에 등록하는 값이라 확정을 기다리는 상태가 아니다 —
     * 남는 사정은 「아직 안 골랐다」 하나뿐이고, 그것이 이 문구다.
     */
  ...
  notes: {
    ...
    lineNoAssignedByServer: '줄번호는 등록할 때 배열 순서대로 매겨집니다.',
    /*
     * ⛔ **「조정 사유 값 목록이 아직 확정되지 않았습니다」가 여기서 걷혔다**(#36 회신 ④).
     *
     * 그 표시는 **우리가 정해야 하는데 못 한 값**에만 쓴다. 조정 사유는 고객이 공통코드
     * 마스터에 등록하는 값이라 그 경우가 아니다 — 사유 칸이 내는 안내는 이제 다섯 참조와
     * 같은 둘뿐이다(`lookups.failed`·`lookups.truncated`).
     */
    /**
