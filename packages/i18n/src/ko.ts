...
const stockAdjust = {
  ...
  actionReasons: {
    ...
    registerVarianceTruncated:
      '조정 등록은 실사 차이를 전부 가져온 뒤에 할 수 있습니다. 지금은 앞쪽 일부만 왔습니다 — 이대로 등록하면 나머지 차이가 조정되지 않은 채 남습니다.',
  ...
  notes: {
    ...
    /**
     * **응답이 오지 않은 요청은 실패가 아니다.** 공통 훅이 호출마다 새 멱등 키를 만들어,
     * 그대로 다시 보내면 서버에는 다른 요청으로 보인다 — 전표가 두 벌 남을 수 있다.
     * **네트워크 갈래에만 붙인다**: 서버가 거절한 요청은 전달된 것이 확실하다.
     */
    networkUnconfirmed:
      '응답이 오지 않았습니다. 보내지지 않았다고 단정할 수 없으니, 다시 보내기 전에 처리 이력에서 같은 조정이 있는지 확인하세요.',
  },
  values: {
    empty: '—',
    unknown: '알 수 없음',
  ...
  result: {
     ...
     */
    unboundCreatedNote: (adjustmentNo: string): string =>
      `앞서 보낸 등록이 끝났습니다 — 조정 전표 ${adjustmentNo}. 지금 세우는 초안과는 별개입니다.`,
  },
} as const;
