...
/**
 * W-01-10(정상품 입하 처리) — **PR ① 몫**(대상 입하 전표·라인 선택).
 *
 * 쓰기·결과·코드 입력 문구는 PR ②에서 이 묶음에 덧붙는다. 지금 미리 적어 두지 않는다 —
 * 쓰이지 않는 문구가 먼저 서 있으면 어느 것이 화면에 실제로 나오는지 읽을 수 없다.
 *
...
const goodsReceipt = {
  ...
  notes: {
    ...
    /*
     * 이 PR에서 화면이 아직 하지 않는 것. **비어 있는 아래쪽이 미완성으로 읽히지 않게** 밝힌다 —
     * 입고 처리 구획은 뒤따르는 작업에서 이 아래에 붙는다.
     */
    postPending: '입고 처리 입력은 아직 이 화면에 없습니다. 지금은 대상을 고르는 것까지 할 수 있습니다.',
  },
} as const;
