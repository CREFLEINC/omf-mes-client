...
const goodsReceipt = {
  ...
  lineTable: {
    ...
    receivedQtyPair: (receivedQty: number, uom: string): string =>
      `${String(receivedQty)} ${uom}`,
  },
  ...
  result: {
    ...
    balanceNote:
      '재고 잔액은 이 화면이 확인하지 않습니다. 잔액은 재고 현황 화면에서 확인하세요.',
    /*
    ...
    erpUnknown:
      'ERP 송신 대기열 적재 여부를 알 수 없습니다. 응답에 그 값이 오지 않았습니다.',
  },
} as const;
...
const stocktaking = {
  ...
  result: {
    ...
    savedNote:
      '이 위치의 라인을 통째로 바꿨습니다. 진행 요약은 위 구획에서 다시 조회한 값입니다.',
  ...
  reasons: {
     ...
     */
    linesTruncated:
      '이 위치의 라인을 전부 받지 못했습니다. 받지 못한 줄이 있어 저장을 막습니다.',
  },
  /*
...
const supplierReturn = {
  ...
  reasons: {
     ...
     */
    balancesFailed:
      '보유 수량을 불러오지 못했습니다. 그 줄의 보유 수량 자리에 사유가 표시됩니다.',
    balancesTruncated:
  ...
  notes: {
     ...
     */
    returnQtyEmptyStart:
      '반품 수량은 빈 칸으로 시작합니다. 되돌려 보낼 양을 줄마다 직접 적으세요.',
    /**
...
const approvalRoute = {
  ...
  empty: {
     ...
     * 오른쪽 구획이 없는 결재선을 가리킨 채 주소만 남는다(수명 표 6행).
     */
  ...
  notes: {
    ...
    inProgressSome: (count: number): string =>
      `진행 중인 요청 ${String(count)}건이 있습니다. 결재선을 고쳐도 이 요청들은 지금의 결재선으로 끝납니다.`,
    inProgressNone: '진행 중인 요청이 없습니다. 결재선을 고쳐도 진행 중인 요청에는 반영되지 않습니다.',
    /**
    ...
    stepGuideRejectResubmit: '진행 중인 요청에 바뀐 결재선을 태우려면 반려한 뒤 다시 상신해야 합니다.',
