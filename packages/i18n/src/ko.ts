...
const login = {
  ...
  actionReasons: {
    /**
     * 로그인 버튼이 잠긴 사유. **「어떻게 풀 것인가」를 담는다**(배치 규범 4 · 공유계약 G-3).
     *
     * 이 파일의 작성 규칙은 「비활성 사유는 그 컨트롤의 이름으로 시작한다」인데, 그 규칙이 막으려는
     * 것은 **사유가 붙은 대상이 시각적으로 끊겼을 때 복원 단서가 없는 것**이다. 이 화면에는 그
     * 거리가 없다 — 컨트롤이 셋뿐이고 사유가 버튼 바로 아래 붙는다. 대신 문장 안에 컨트롤 이름
     * (「로그인」)을 담아 단서를 남기고, 사용자가 지금 할 수 있는 조치를 문장 앞에 둔다.
     *
     * **어느 칸이 비었는지 가르지 않는다.** 세 갈래(아이디만·비밀번호만·둘 다 공백만)가 같은
     * 문장을 쓴다.
     */
    incomplete: '아이디와 비밀번호를 모두 입력하면 로그인할 수 있습니다.',
  },
} as const;
