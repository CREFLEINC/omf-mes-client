...
const login = {
  ...
  actionReasons: {
     ...
     *
     * 규범 4-5가 지정한 두 마디 문형(「…없습니다. …이면 이 버튼을 쓸 수 있습니다」)까지는 따르지
     * 않는다. 이 화면에서는 **막힌 사정과 푸는 방법이 같은 사실**(칸이 비었다)이라 두 마디로
     * 나누면 같은 말을 두 번 하게 된다 — 규범이 지키려는 것(주어가 있고 조치가 보인다)은 한 마디로
     * 충족된다.
     *
