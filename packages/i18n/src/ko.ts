...
const stockAdjust = {
  ...
  historyFilters: {
    ...
    all: '전체',
    /**
     * 값 목록이 확정되지 않은 코드 칸(D-9 · 미결 #64).
     *
     * **등록 쪽 문구와 갈라 둔다** — 저쪽은 「값이 확정되면 등록할 수 있습니다」로 등록의
     * 잠금을 말하지만, 이력 조회는 **비어 있어도 아무것도 막히지 않는다.** 같은 문구를 쓰면
     * 사용자가 조회까지 막힌 줄 안다.
     */
   ...
   *
   * **코드를 뜻으로 옮기지 않는다**(공유계약 G-2). 사유·상태의 값 목록이 확정되지 않아
   * 화면이 뜻을 붙이면 값이 정해질 때 조용히 틀린다.
   */
  historySummary: {
    label: '조정 전표 요약',
    inventoryAdjustmentNo: '조정 전표번호',
