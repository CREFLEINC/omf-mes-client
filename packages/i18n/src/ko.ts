...
const stockAdjust = {
   ...
   *
   * 헤더 사유는 **코드**(고르는 값 · 값 목록 미확정)이고 이것은 **자유 텍스트**(쓰는 글)다.
  ...
  actionReasons: {
    ...
    /** 고를 값 자체가 없다 — 「아직 안 골랐다」와 다른 사정이라 문구를 가른다. */
    registerReasonPending:
      '조정 등록은 조정 사유 값 목록이 확정된 뒤에 할 수 있습니다. 값이 확정되면 이 화면에서 등록할 수 있습니다.',
  ...
  notes: {
     ...
     * **네트워크 갈래에만 붙인다** — 서버가 거절한 요청은 전달된 것이 확실하다.
     *
     * **없는 자리를 가리키지 않는다.** 이 슬라이스에는 아직 조정을 조회할 자리가 없고(처리
     * 이력은 뒤따르는 회차) 라우트도 닫혀 있다 — 확인하러 갈 자리를 지어내면 「할 수 없는
     * 조치」를 지시하게 되고, 그 순간 이 완화가 무력해진다. 그래서 **확인할 수 없다는 사실과
     * 하지 말아야 할 일**을 말한다. 이 갈래에서 전표 두 벌을 막는 것은 확인이 아니라 **금지**다 —
     * 금지를 선행 확인에 매달면 「확인하면 다시 보내도 된다」로 읽힌다.
     *
     * ⚠ **처리 이력 탭이 서면 이 문구를 그 탭을 가리키도록 갱신한다.**
     */
    networkUnconfirmed:
      '응답을 받지 못해 조정 전표가 만들어졌는지 이 화면에서 확인할 수 없습니다. 이미 만들어졌을 수 있으니 같은 내용을 바로 다시 등록하지 마세요.',
  },
  values: {
    empty: '—',
    unknown: '알 수 없음',
  ...
  result: {
     ...
     * 모르는 전표가 서버에 남는다)가 그대로 되돌아온다. 이 슬라이스에는 아직 그 번호를 되찾을
     * 조회 자리가 없다(처리 이력은 뒤따르는 회차).
     */
    unboundCreatedNote: (adjustmentNos: string): string =>
      `앞서 보낸 등록이 끝나 조정 전표가 만들어졌습니다 — ${adjustmentNos}. 지금 세우는 초안과는 별개입니다.`,
    /**
    ...
    unboundSubmittedNote: (adjustmentNos: string): string =>
      `앞서 보낸 상신이 끝나 조정 전표가 결재에 올라갔습니다 — ${adjustmentNos}. 지금 보고 있는 것과는 별개입니다.`,
  },
  /**
  ...
  post: {
     ...
     * 이 문구뿐이다.**
     *
     * **두 절로 적는다**(등록 축 `notes.networkUnconfirmed`와 같은 잣대): ① 이 화면에서
     * **확인할 수 없다**는 사실 ② **하지 말아야 할 일**. 확인 절차를 지시하지 않는다 — 이
     * 슬라이스에는 아직 전기 여부를 되찾을 조회 자리가 없고(처리 이력은 뒤따르는 회차),
     * 금지를 선행 확인에 매달면 「확인하면 다시 보내도 된다」로 읽힌다.
     *
     * ⚠ **처리 이력 탭이 서면 이 문구를 그 탭을 가리키도록 갱신한다.**
     */
    networkUnconfirmed:
      '응답을 받지 못해 재고가 움직였는지 이 화면에서 확인할 수 없습니다. 이미 원장에 잡혔을 수 있으니 같은 전표를 바로 다시 전기하지 마세요.',
    /**
    ...
    unboundPostedNote: (adjustmentNos: string): string =>
      `앞서 보낸 전기가 끝나 재고가 움직였습니다 — ${adjustmentNos}. 지금 보고 있는 것과는 별개입니다.`,
  },
  /**
