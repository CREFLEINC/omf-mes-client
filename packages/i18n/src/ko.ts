...
const commonCode = {
  ...
  partnerRole: {
     ...
     * 풀리는** 자원을 전제로 한다. 거래처 역할 치환은 계약이 이 자원의 어느 응답에도 `ETag`를
     * 선언하지 않아 토큰이 오지 않으며, 다시 눌러도 같은 자리에서 멈춘다 — 공통 문구를 그대로
     * 쓰면 **없는 조치를 지시하는** 안내가 된다.
     *
     * 사용자에게 할 일을 시키지 않는다. 지금 상태가 무엇인지만 밝히고, 되풀이해도 달라지지
     * 않는다는 사실을 함께 적어 헛된 시도를 막는다. 공통 문구 자체는 고치지 않는다 —
     * 다시 시도가 실제로 통하는 형제 화면에서는 그 말이 참이다.
     */
    saveTokenUnavailable:
      '저장에 필요한 정보가 서버에서 아직 제공되지 않아 지금은 저장할 수 없습니다. 다시 눌러도 같은 결과입니다.',
    actionReasons: {
