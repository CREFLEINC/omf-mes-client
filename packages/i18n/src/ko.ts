...
const documentProgress = {
  ...
  notes: {
     ...
     *
     * 취소 요청이 나가는 동안 조회·초기화·쪽 이동·고르기·해제가 전부 잠기는데, 그 손잡이 중
     ...
     *
     * ⚠ **취소 실행이 붙는 회차(단위 ④)에 이 문면을 다시 본다.** 실행도 같은 잠금을 쓰므로
     * 「취소 요청」만 말하는 지금 문면은 **실행 중에 거짓**이 된다 — 그때 두 조작이 함께 읽히게
     * 고치거나 갈래를 나눈다.
     */
    cancelLock: '보내는 중인 취소 요청이 끝날 때까지 다시 조회하거나 다른 문서로 옮길 수 없습니다.',
  },
  /**
