...
export const usersRoles = {
  ...
  user: {
    ...
    validation: {
      ...
      initialPasswordWeak: (minLength: number): string =>
        `초기 비밀번호는 숫자와 알파벳을 함께 넣어 ${String(minLength)}자 이상이어야 합니다.`,
    },
  },
  /**
  ...
  permission: {
    editNote:
      '칸을 눌러 권한을 주거나 거둔 뒤 저장하세요. 저장은 이 역할의 권한 전체를 한 번에 바꿉니다.',
    ...
    /* 표의 접근 설명. 무엇을 담은 표인지 한 줄로 말한다. */
    tableCaption: (roleLabel: string) => `${roleLabel} 역할의 기능 권한`,
    /*
     * 고르기만 하고 저장하지 않는다 - 그래서 「저장」과 나란히 두지 않고 표 위에 둔다.
     * 되돌리는 길은 「취소」다.
     */
    selectAll: '전체 선택',
    clearAll: '전체 해제',
    /*
     * 묶음 이름 - groupCode 는 개체 식별자라 서버가 이름을 주지 않는다(계약 x-no-code-key).
     * 고정 설계의 권한 목록 생성기가 적어 둔 축 7개의 이름을 옮긴 것이고,
     * 모르는 축이 오면 화면이 코드를 그대로 낸다 - 없는 이름을 지어내지 않는다.
     */
    groups: {
      byCode: {
        '01': '자재·창고',
      ...
      /* 계약이 groupCode 를 선택으로 두어 빠질 수 있다. 빠진 것을 다른 묶음에 섞지 않는다. */
      ungrouped: '묶음 없음',
      unlisted: '목록에 없는 권한',
    },
    /*
     * 후보에 없는데 부여돼 있는 열. 서버가 치환에서 후보 밖 코드를 거부하므로 정상적으로는
     * 생기지 않는다 - 생겼다는 사실 자체를 알려야 저장했을 때의 거부를 미리 이해한다.
     */
    unlistedNotice:
      '부여할 수 있는 권한 목록에 없는 권한이 이 역할에 남아 있습니다. 「목록에 없는 권한」 묶음에 두었고, 그대로 저장하면 서버가 되돌릴 수 있습니다.',
    /*
     * 마지막 관리자 차단. 서버가 code=LAST_ADMIN 으로 거부한다.
     * 「왜」가 아니라 「어떻게 풀 것인가」를 담는다(공유계약 G-3).
     */
    lastAdmin: {
      title: '이 권한을 가진 사람이 없어집니다',
      description:
        '「사용자·역할·권한 관리」 권한을 가진 사람이 한 명도 남지 않아 저장하지 않았습니다. 다른 역할에 이 권한을 먼저 준 뒤 다시 거두세요.',
    },
    /*
    ...
    notGranted: '부여되지 않음',
    empty: {
      /* 부여가 0건인 것이 아니라 «줄 수 있는 후보»가 0건인 상태다 - 그 둘을 가려 말한다. */
      none: '부여할 수 있는 기능 권한이 없습니다',
      noneDescription: '권한 목록을 불러오지 못했거나 비어 있습니다. 잠시 뒤 다시 조회하세요.',
    },
  },
  /**
