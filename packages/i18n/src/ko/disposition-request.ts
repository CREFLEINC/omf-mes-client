...
export const dispositionRequest = {
  ...
  /**
   * 부적합 상태 조건을 골랐을 때의 안내 — 통보 186 · 공유계약 L-3-2 ⑷.
   *
   * ⛔ **「없습니다」로 말하지 않는다.** 적체 관리 화면에서 빈 목록은 「미처리 건이 없다」로
   * 읽히는데, 실제로는 부를 수 없을 뿐 건은 남아 있다. 「불러올 수 없다」와 그 사유를 말한다.
   */
  periodContractBlocked:
    '부적합 상태로는 아직 목록을 불러올 수 없습니다. 서버가 조회 기간을 필수로 요구하는데 이 화면에는 기간 조건이 없어, 서버팀 확인 뒤 열립니다. 그동안은 상태 조건을 비우고 판정 대상 목록을 이용하세요.',
  panes: {
    list: '판정 대기 대상',
    target: '선택한 대상',
    register: '① 부적합 등록',
