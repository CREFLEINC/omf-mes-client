...
export const repackLabelIssue = {
  ...
  pending: {
    ...
    unknown: '—',
    /**
     * 이 포장을 만든 재구성 사건이 없는 줄의 첫 칸(#1044 · #1147 → #1150).
     *
     * ⛔ **아무 말도 하지 않는 줄을 세우지 않는다.** 한때 이 줄은 포장 번호조차 없이 모든 칸이
     * 「—」 라, 작업자가 눌러 보기 전에는 무엇인지 알 수 없었다. 번호는 늘 오므로 적고, 나머지가
     * 왜 비었는지도 함께 적는다.
     *
     * ⭐ **「없음」은 서버가 사건 0건으로 답했을 때만이다**(#1150). 조회가 실패하면 목록 전체가
     *    `loadFailed` 로 따로 말한다. 한때 이 경우까지 「확인 불가」로 적어 못 읽은 것처럼 보였다.
     */
    noEvent: (handlingUnitNo: string): string => `${handlingUnitNo} · 재구성 이력 없음`,
    /**
     * 사건은 있는데 **이 포장을 새로 만든 쪽이 아닌** 줄(원 포장·잔량으로 쓰였던 포장).
     *
     * ⛔ **「없다」로 말하지 않는다** — 이력은 있다. 이 포장이 왜 발행 대기에 섰는지를 화면이
     *    사건에서 읽어 낼 수 없을 뿐이다. 없는 것과 모르는 것을 같은 말로 섞지 않는다(전례
     *    `progressUnknown`).
     */
     ...
     *
     * ⚠ 재구성 사건이 없거나 못 맞춘 줄도 이것을 탄다 — 그때 `sourceText` 가 `noEvent`·`unknownEvent` 다.
    ...
    /* 다시 시도해도 풀리지 않는다 — 서버가 이 목록의 조건을 아직 제공하지 않는다(#1095). */
    unsupported:
      '발행 대기 목록은 아직 서버가 지원하지 않습니다. 서버가 준비되면 이 화면에서 대상을 고를 수 있습니다.',
  },
