...
export const repackLabelIssue = {
  ...
  pending: {
     ...
     * 왜 비었는지도 함께 적는다.
     *
     * ⭐ **「없음」이 맞다 — 「확인 불가」가 아니다**(#1150). 이 줄에 닿는 것은 서버가 사건 조회에
     *    **200 으로 답했는데** 이 포장을 새로 만든 사건이 없을 때뿐이다. 조회가 실패하면 목록
     *    전체가 `loadFailed` 로 따로 말한다. 한때 「확인 불가」로 적어 못 읽은 것처럼 보였다 —
     *    없는 것과 모르는 것을 같은 말로 섞지 않는다(전례 `progressUnknown`).
     */
     ...
     *
     * ⚠ 재구성 사건이 없는 줄도 이것을 탄다 — 그때 `sourceText` 가 `noEvent` 다.
