export const shippingPackingLabel = {
  ...
  shipment: {
     ...
     */
    missing: '출하 포장에서 출하 대상을 먼저 선택하세요.',
  },
  kind: {
    legend: '라벨 종류',
    packing: '포장라벨',
  ...
  actions: {
    ...
    /*
     * ⭐ **종류를 고르기 전에도 이유를 말한다**(#1094 · 88단계 2회차 실기).
     *
     * 종전에는 이 상태에서 발행 버튼이 꺼진 채 아무 말도 하지 않았다 — 무엇을 해야 열리는지
     * 알 길이 없어 현장은 고장으로 읽었다. 대상보다 «먼저» 고르는 칸이라 순서도 앞이다.
     *
     * ⛔ **`targets.beforeKind` 와 같은 문장을 쓰지 않는다.** 대상 목록이 이미 「라벨 종류를
     *    먼저 고르세요」를 세우고 있어, 같은 말을 버튼 옆에 한 번 더 두면 한 화면에 같은
     *    문장이 둘 선다. 이 자리는 **그래서 버튼이 안 열린다**를 말한다.
     */
    needsKind: '라벨 종류를 고르면 발행할 수 있습니다.',
