...
export const equipmentMaster = {
   ...
   * ⚠ **이 목록에 계측기가 함께 보인다.**
   *
   * `equipmentTypeCode` 한 컬럼에 두 계열이 착지하고, 계약은 이제 값 여럿을 받는데
   * (통지 `client#404`) **이 화면이 실을 설비 계열 값 목록이 아직 미정**이다
   * (설계 `omf-mes#145`). 형제 화면(W-05-11)은 계측기 계열 값이 확정돼 조건을 걸 수 있다.
   *
   * ⛔ **받아 온 목록을 화면에서 거르지 않는다**(공유계약 L-1) — 서버가 페이지로 잘라 주는데
   * 그 안에서 또 거르면 건수와 「더 있음」 안내가 통째로 거짓이 된다. **감추는 대신 밝힌다**(G-2).
   */
