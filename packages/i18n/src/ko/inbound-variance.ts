...
export const inboundVariance = {
  ...
  receipt: {
    ...
    lineLabel: (no: number, qty: string) => `${String(no)}번 라인 · 실입하 ${qty}`,
    /* 카드로 세우면 제목과 수치의 무게가 갈린다. 단추 안에 넣으면 둘이 같은 크기로 선다. */
    lineNo: (no: number) => `${String(no)}번 라인`,
    lineQty: (qty: string) => `실입하 ${qty}`,
    linePicked: '선택됨',
    chosen: (no: number, qty: string) => `고른 라인 ${String(no)}번 · 실입하 ${qty}`,
    change: '다른 대상 고르기',
