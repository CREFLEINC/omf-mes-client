...
export const materialPicking = {
  ...
  /** 계획까지 다 집어 내보낸 경우. **지시가 끝났다**는 뜻이다. */
  allIssued: '이 지시에서 내보낼 것이 남아 있지 않습니다.',
  /*
   * ⛔ 부분 출고 뒤에는 「끝났다」고 말하지 않는다 — 계획에 남은 수량이 있는데 끝났다고
   * 읽히면 작업자가 그대로 나간다(omf-all-around#49). 사실은 「집어 둔 것을 다 내보냈다」이고,
   * 다음에 할 일을 함께 적는다(공유계약 G-3).
   *
   * 남은 것은 **라인 건수**로 센다 — 라인마다 단위가 달라 수량 합계는 뜻이 서지 않는다.
   */
  allPickedIssued: (unpickedLines: number): string =>
    `집어 둔 수량은 모두 출고했습니다. 아직 피킹이 필요한 라인이 ${String(unpickedLines)}건 남아 있습니다. 다시 피킹한 후 확정해주세요.`,
