...
export const materialInputScan = {
  ...
  notes: {
    ...
    /*
     * 「계획 대비 수령」 구획의 도움말. ⚠·⛔ 가 붙은 줄을 보고 「그럼 못 하는 건가」를
     * 의심한 사람에게만 보이면 되는 말이라, 상시 문구가 아니라 도움말 버튼에 담는다.
     */
    /*
     * ⚠ **줄 나눔을 글에 담는다.** 이 말은 말풍선에 서는데, 상자 폭을 «상한»으로만 묶으면
     * 접힌 줄이 짧아도 상자는 늘 상한만큼 서서 오른쪽이 빈다(실측). 줄이 어디서 나뉘는지
     * 정해 주면 상자가 «가장 긴 줄»에 맞춰진다. 화면이 1024×768 로 고정이라 가능한 방법이다.
     *
     * ⛔ 문장을 바꿀 때 줄 나눔도 함께 본다 — 한쪽만 고치면 상자가 다시 벌어진다.
     */
    shortAllowed: '부족·미수령이 있어도 수령한\n양으로 투입할 수 있습니다.',
    /** 도움말 버튼의 이름 — 글자가 「?」 하나뿐이라 보이는 이름을 대신할 것이 필요하다. */
    shortAllowedLabel: '수령이 모자랄 때 어떻게 되는지',
