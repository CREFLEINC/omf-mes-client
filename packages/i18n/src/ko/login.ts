...
export const login = {
  ...
  /**
   * 실패 안내 — **두 가지만 말한다**(#1034).
   *
   * **화면 수준 배너로만 낸다** — 칸 옆에 붙은 오류는 문구를 아무리 뭉뚱그려도 **붙은 자리가
   * 어느 칸이 문제인지 말한다**(공유계약 G-1 · F-7).
   *
   * ⚠ **앞 회차의 남은 시도 횟수 안내(`(2/5)`)와 잠긴 계정 문구를 지웠다.** 실패를 둘로 줄이며
   * 함께 사라진 자리다. 되살리려면 갈래를 늘리는 결정이 먼저다 — 문구만 되돌리면 그릴 갈래가 없다.
   */
  banner: {
    /**
     * 로그인 정보 오류의 제목. **무엇이 잘못됐는지를 제목이 말한다** — 두 갈래를 제목만 보고도
     * 가를 수 있어야 배너가 둘로 줄어든 뜻이 산다.
     */
