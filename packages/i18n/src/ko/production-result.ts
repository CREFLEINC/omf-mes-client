...
export const productionResult = {
  ...
  quantity: {
     ...
     *
     * ⭐ **이름을 «두 자리»가 나눠 맡는다.** 설계는 이 줄도 다른 줄과 같은 「라벨 | 값」 격자에
     * 세운다(검증본 `.col-l` 마지막 행 — 「잔여수량 | 380 / 500 EA」). 라벨이 앞 숫자의
     * 이름이고, 값은 뒤 숫자에만 「지시」를 붙인다.
     *
     * ⛔ 값에 「잔여」를 다시 넣지 않는다 — 라벨과 겹쳐 「잔여수량 잔여 150 …」이 된다
     * (실측 · 사용자 지적).
     */
    remaining: '잔여수량',
    remainingValue: (remaining: string, ordered: string) => `${remaining} / 지시 ${ordered}`,
