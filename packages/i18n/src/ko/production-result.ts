...
export const productionResult = {
  ...
  flow: {
    ...
    quantity: {
      ...
      /**
       * 잔여를 넘겼을 때 **막지 않고 말하는** 자리(#1040).
       *
       * ⭐ **초과 생산은 허용으로 확정됐다** — R27 · ✓확정 QA #27 · `P-02-06` §5-4 「초과 생산 —
       * 막지 않는다 · 화면은 「초과 달성」으로 표시하고 완료를 허용한다」. 낱말을 그 문서에서
       * 그대로 가져온다.
       *
       * ⛔ **「지시」라고 적지 않는다** — 뒤 숫자에 그 낱말을 덧대 읽기 어려워진 지적이 두 번
       * 있었다(같은 파일 `quantity.remaining` 머리말). 이 화면이 재는 축은 잔여이므로 잔여로
       * 적는다.
       */
