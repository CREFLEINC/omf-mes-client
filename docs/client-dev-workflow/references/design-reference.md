...
그 뒤 변경 지점을 직접 분석하고 현재 계획·구조 설계·코드·테스트 영향 범위를 전면 재검토한다. 결과는 현재 작업 이슈 또는 로컬 계획에 기록한다. 공지 형식은 설계팀이 소유하므로 개발팀 저장소에는 공지·영향 검토 이슈 템플릿을 만들지 않으며, 자세한 판단은 지정 버전의 자료에서 수행한다.
