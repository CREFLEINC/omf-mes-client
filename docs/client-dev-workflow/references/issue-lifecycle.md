...
설계 변동 공지 원본은 백엔드·클라이언트를 구분하지 않는 공통 채널에 있어야 한다. 클라이언트 저장소에는 공지를 재발행하지 않고 `.github/ISSUE_TEMPLATE/design-change-impact-review.yml`로 공통 공지 포인터와 클라이언트 영향 검토 결과만 남긴다.
