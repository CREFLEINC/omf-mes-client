...
설계팀의 직접 통지는 모든 개발팀을 대상으로 한 **설계 변동 공지**만 수신한다. 공지의 형식·작성·발행은 설계팀이 결정한다. 개발팀은 공지 형식을 정의하거나 보완하지 않으며, 백엔드·클라이언트별 공지로 재작성하거나 이 저장소에 원문을 복사하지 않는다.
...
5. 공통 공지 포인터와 재검토 결과는 현재 작업 이슈 또는 로컬 계획에 기록한다. 별도의 공지·영향 검토 이슈 템플릿은 만들지 않는다.
