...
const issueQtyOf = (issue) => {
  ...
  const line = state.goodsIssueLines.find(
    (row) => row.goodsIssueLineId === Number(issue.targetId),
  );
...
on('GET', '/app/document-issues/{documentIssueLogId}/rendition', (params, query) => {
  ...
  /*
   * ⚠ **`tspl` 은 계약에 아직 없다**(#891 — 서버 구현 대기). 그림으로 내려주면 단말이 그것을
   *   드라이버에 넘겨 그리므로 프린터 글꼴이 아니라 픽셀이 찍힌다 — 명령(`Ctrl+Alt+P`)으로
   *   뽑은 라벨과 «다른 물건»으로 보였다(실측 2026-09-08). 시험에서 같은 라벨을 보려고
   *   여기서 먼저 내려준다(사용자 지시). 계약에 형식이 서면 이 갈래는 그쪽으로 옮긴다.
   */
